...
const KEY_ALL_DAYS = "shifto_alldays_v293"; const KEY_MONTHLY = "shifto_monthly_v293"; const KEY_RULES = "shifto_rules_v293";
...
const RCard = ({ bg, b, c, ic, t, d, children }: any) => ( <div style={{ background: bg, padding: 24, borderRadius: 12, border: `2px solid ${b}`, marginBottom: 20 }}><h5 style={{ margin: "0 0 12px 0", color: c, fontSize: 18, fontWeight: 800 }}>{ic} {t}</h5>{d && <p style={{ fontSize: 15, color: "#166534", marginTop: 0, marginBottom: 16 }}>{d}</p>}{children}</div> );
...
const StaffSel = ({v, onChange, list, ph="スタッフ", w}:any) => <select value={v} onChange={e=>onChange(e.target.value)} className="rule-sel" style={{width:w, flex:"0 0 auto"}}><option value="">{ph}</option>{list.map((s:any)=><option key={s} value={s}>{s}</option>)}</select>;
const RoomSel = ({v, onChange, list, ph="場所", w}:any) => <select value={v} onChange={e=>onChange(e.target.value)} className="rule-sel" style={{width:w, flex:"0 0 auto"}}><option value="">{ph}</option>{list.map((s:any)=><option key={s} value={s}>{s}</option>)}</select>;
...
export class AutoAssigner {
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number) {
    ...
    const eff = this.getEffectiveTarget(section, targetCount); if (eff.al) return;
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const getFilterReason = (name: string): RejectReason | null => { if (current.map(extractStaffName).includes(name)) return { hard: true, msg: "同室配置済" }; if (this.isUsed(name)) return { hard: true, msg: "他業務配置済" }; if (this.isForbidden(name, section)) return { hard: true, msg: "担当不可" }; if (section === "MMG" && !this.isMmgCapable(name)) return { hard: true, msg: "MMG外" }; if (!this.canAddKenmu(name, section)) return { hard: true, msg: "兼務上限" }; const b = this.blockMap.get(name); if (needTag && b === 'NONE') { if (!eff.pC && !eff.aC && !isMonthlyMainStaff(section, name, this.ctx.monthlyAssign)) return { hard: true, msg: "半端枠" }; } if (b === 'ALL') return { hard: true, msg: "全日ブ" }; if (needTag === "(AM)" && b === 'AM') return { hard: true, msg: "AMブ" }; if (needTag === "(PM)" && b === 'PM') return { hard: true, msg: "PMブ" }; if (eff.pC && b === 'AM') return { hard: true, msg: "午後休" }; if (eff.aC && b === 'PM') return { hard: true, msg: "午前休" }; if (this.noHD(section, needTag || b || "")) return { hard: true, msg: "終日専任" }; if (this.isHalfDayBlockedForFullDayRoom(name, section).hard) return { hard: true, msg: "終日専任" }; if (this.isHardNoConsecutive(name, section)) return { hard: false, msg: "連日禁止" }; if (this.hasNGPair(name, current.map(extractStaffName), false)) return { hard: true, msg: "絶対NG" }; if (this.hasNGPair(name, current.map(extractStaffName), true)) return { hard: false, msg: "なるべくNG" }; return null; };
  ...
  processPostTasks() {
    ...
    (this.ctx.customRules.swapRules || []).forEach((r: any) => {
      ...
      if (!tM.some(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, r.targetRoom) && !this.isHardNoConsecutive(c, r.targetRoom) && !this.isHB(c, r.targetRoom).h && this.canAddKenmu(c, r.targetRoom, true); })) {
        let sw = false;
        for (const sSR of split(r.sourceRooms).sort((a, b) => this.getRescueSourceScore(parseRoomCond(a).r, r.targetRoom) - this.getRescueSourceScore(parseRoomCond(b).r, r.targetRoom))) {
          ...
          let sC = sM.filter(m => !ROLE_PLACEHOLDERS.includes(extractStaffName(m)) && !this.isForbidden(extractStaffName(m), r.targetRoom) && !this.isHB(extractStaffName(m), r.targetRoom).h && !this.isHardNoConsecutive(extractStaffName(m), r.targetRoom) && this.canAddKenmu(extractStaffName(m), r.targetRoom, true) && !this.isForbidden(extractStaffName(m), r.triggerRoom) && !this.noHD(r.targetRoom, m));
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((r: any) => {
      ...
      while ((cA < tC || cP < tC) && uG1.length > 0) { const ci = uG1.findIndex(s => !this.isForbidden(s, r.target) && !this.isHB(s, r.target).h && !this.hasNGPair(s, cM.map(extractStaffName), false) && !this.isHardNoConsecutive(s, r.target) && !(r.target === "MMG" && !this.isMmgCapable(s)) && this.canAddKenmu(s, r.target) && !(cP >= tC && this.blockMap.get(s) === 'AM') && !(cA >= tC && this.blockMap.get(s) === 'PM') && !this.noHD(r.target, this.blockMap.get(s) || "")); if (ci === -1) break; const st = uG1[ci]; uG1.splice(ci, 1); const b = this.blockMap.get(st); let t = b === 'AM' ? "(PM)" : b === 'PM' ? "(AM)" : ""; if (!t) { if (cA >= tC) t = "(PM)"; else if (cP >= tC) t = "(AM)"; } this.blockMap.set(st, t === "" ? 'ALL' : t === "(AM)" ? 'PM' : 'AM'); cM.push(`${st}${t}`); if (t === "(AM)") cA++; else if (t === "(PM)") cP++; else { cA++; cP++; } this.addU(st, t ? 0.5 : 1); this.log(`🙌 [負担軽減] 余剰の ${st} を ${r.target} に専任配置`); } this.dayCells[r.target] = join(cM);
    });
    ...
    (this.ctx.customRules.smartKenmu || []).forEach((r: any) => {
      ...
      for (const tm of tM) { const tc = extractStaffName(tm); if (ROLE_PLACEHOLDERS.includes(tc) || ROOM_SECTIONS.some(rm => rm !== r.targetRoom && split(this.dayCells[rm]).map(extractStaffName).includes(tc))) continue; 
        ...
        for (const sSR of split(r.sourceRooms)) { const { r: sR, min } = parseRoomCond(sSR); const sMs = split(this.dayCells[sR]); if (sR === "透析後胸部" || (min > 0 && sMs.reduce((s, m) => s + getStaffAmount(m), 0) < min) || this.isForbidden(tc, sR) || this.isHB(tc, sR).h || !this.canAddKenmu(tc, sR, true)) continue;
          let cnds = sMs.filter(m => { const c = extractStaffName(m); return c !== tc && !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, r.targetRoom) && !this.hasNGPair(c, tM.map(extractStaffName), false) && !this.isHardNoConsecutive(c, r.targetRoom) && (r.targetRoom === "MMG" ? this.isMmgCapable(c) : true) && this.canAddKenmu(c, r.targetRoom) && !this.noHD(r.targetRoom, m); });
        ...
        if (sC && fSR) { const cc = extractStaffName(sC); this.dayCells[fSR] = join([...split(this.dayCells[fSR]).filter(m => m !== sC), `${tc}${tm.includes("(AM)") ? "(AM)" : tm.includes("(PM)") ? "(PM)" : ""}`]); this.dayCells[r.targetRoom] = join(tM.map(m => m === tm ? sC : m) as string[]); this.addU(cc, getStaffAmount(sC as string)); this.blockMap.set(tc, tm.includes("(AM)") ? 'PM' : tm.includes("(PM)") ? 'AM' : 'ALL'); break; }
      }
    });

    const processKenmu = (sm: string[], tm: string[], tr: string) => { if (tr === "透析後胸部") return tm; const tC = this.dynamicCapacity[tr] || 1; const tcs = tm.map(extractStaffName); let cA = tm.reduce((s, m) => s + getStaffAmount(m), 0); for (const m of sm) { if (cA >= tC) break; const c = extractStaffName(m); if (tcs.includes(c) || m.includes("17:") || m.includes("19:") || this.isForbidden(c, tr) || this.isHardNoConsecutive(c, tr) || this.isHB(c, tr).h || this.hasNGPair(c, tcs, false) || !this.canAddKenmu(c, tr) || this.noHD(tr, m)) continue; let pS = m, cam = 0, cpm = 0; tm.forEach(x => { if (x.includes("(AM)")) cam++; else if (x.includes("(PM)")) cpm++; else { cam++; cpm++; } }); if (cam < tC && cpm >= tC) { if (m.includes("(PM)")) continue; pS = `${c}(AM)`; } else if (cam >= tC && cpm < tC) { if (m.includes("(AM)")) continue; pS = `${c}(PM)`; } tm.push(pS); tcs.push(c); const a = getStaffAmount(pS); cA += a; this.addU(c, a); this.updateBlockMapAfterKenmu(c, pS); } return tm; };
    ...
    (this.ctx.customRules.emergencies || []).forEach((em: any) => {
      ...
      let sw = false; for (const sF of sRL) { if (sw || sF === "透析後胸部") break; const sM = split(this.dayCells[sF]); if (!sM.length) continue; const ngI = sM.filter(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && (this.isForbidden(c, wR) || !this.canAddKenmu(c, wR, true)); }); if (!ngI.length) continue; for (const src of ROOM_SECTIONS.filter(r => r !== wR && r !== sF && !this.skipSections.includes(r) && !["待機", "昼当番", "受付", "受付ヘルプ", "透析後胸部"].includes(r))) { if (sw) break; const rM = split(this.dayCells[src]); const oC = rM.filter(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, wR) && !this.isForbidden(c, sF) && !this.isHB(c, wR).h && !this.isHardNoConsecutive(c, wR) && this.canAddKenmu(c, wR, true) && !m.includes("17:") && !m.includes("19:") && !this.noHD(wR, m); }); for (const om of oC) { const oc = extractStaffName(om); const km = ngI.find(m => { const c = extractStaffName(m); return !this.isForbidden(c, src) && !this.isHB(c, src).h && !this.hasNGPair(c, rM.map(extractStaffName), false) && this.canAddKenmu(c, src, true); }); if (!km) continue; const kc = extractStaffName(km); this.dayCells[sF] = join(sM.map(m => m === km ? m.replace(kc, oc) : m)); this.dayCells[src] = join(rM.map(m => m === om ? m.replace(oc, kc) : m)); sw = true; break; } } }
    });
    ...
    uG2.forEach(st => {
      ...
      for (const rm of dKT) {
        if (this.skipSections.includes(rm) || this.isForbidden(st, rm) || (rm === "MMG" && !this.isMmgCapable(st))) continue; if (t !== "" && this.isHB(st, rm).h) continue; const e = this.gET(rm, 1); if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC)) continue;
          ...
          <div style={{ borderLeft: "8px solid #94a3b8", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RC bg="#fef2f2" b="#fecaca" c="#b91c1c" ic="🚫" t="NGペア" ch={
              <>{(customRules.ngPairs || []).map((rule: any, idx: number) => (
                  <Row key={idx}><StaffSel v={rule.s1} oC={(v:any)=>updateRule("ngPairs", idx, "s1", v)} list={activeGeneralStaff} /><span className="rule-label">と</span><StaffSel v={rule.s2} oC={(v:any)=>updateRule("ngPairs", idx, "s2", v)} list={activeGeneralStaff} /><select value={rule.level || "hard"} onChange={(e: any) => updateRule("ngPairs", idx, "level", e.target.value)} className="rule-sel" style={{flex:"0 0 auto", minWidth:"120px"}}><option value="hard">絶対NG</option><option value="soft">なるべくNG</option></select><Del oC={()=>removeRule("ngPairs", idx)} /></Row>
              ...
              <>{(customRules.smartKenmu || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ background: "#fff", padding: "16px 20px", border: "1px solid #f0abfc", borderRadius: 8, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 6 }}><MP v={rule.sourceRooms} oC={(v: string) => updateRule("smartKenmu", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
              ...
              {(customRules.linkedRooms || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #a7f3d0", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 10 }}><MP v={rule.sources} oC={(v: string) => updateRule("linkedRooms", idx, "sources", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
              ...
              {(customRules.rescueRules || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #fde047", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10 }}><MP v={rule.sourceRooms} oC={(v: string) => updateRule("rescueRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flexShrink: 0, alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1, minWidth: "260px", background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff" }}>
                    ...
                    {(customRules.lunchSpecialDays || []).map((rule: any, idx: number) => (
                      <div key={idx} className="rule-row">
                        ...
                        <span className="rule-label">は</span><Num v={rule.count} oC={(v:any)=>updateRule("lunchSpecialDays", idx, "count", v)} /><Del oC={()=>removeRule("lunchSpecialDays", idx)} />
                      </div>
                    ))}
                  ...
                  <div style={{ flex: 1, minWidth: "260px", background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff" }}>
                    ...
                    {(customRules.lunchConditional || []).map((rule: any, idx: number) => (
                      <div key={idx} className="rule-row">
                        ...
                        <span className="rule-label">人以上➔</span><Num v={rule.out} oC={(v:any)=>updateRule("lunchConditional", idx, "out", v)} /><Del oC={()=>removeRule("lunchConditional", idx)} />
                      </div>
                    ))}
