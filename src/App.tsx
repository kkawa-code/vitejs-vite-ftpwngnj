...
export function parseAndSortStaff(staffString: string) { const list = split(staffString); const parsed = list.map(s => { const match = s.match(/^(.*?)[\(（](.*?)[\)）]$/); return { cleanName: match ? match[1].trim() : s, yomi: match ? match[2].trim() : s }; }); parsed.sort((a, b) => a.yomi.localeCompare(b.yomi, 'ja')); return Array.from(new Set(parsed.map(p => p.cleanName))); }
export function formatDayForDisplay(d: Date) { const YOUBI = ["日", "月", "火", "水", "木", "金", "土"]; return `${d.getMonth() + 1}/${d.getDate()}(${YOUBI[d.getDay()]})`; }
export function getMonthlyStaffForSection(sec: string, monthlyAssign: Record<string, string>): string[] { let staff: string[] = []; if (sec === "治療") staff = [...split(monthlyAssign.治療), ...split(monthlyAssign.治療サブ優先), ...split(monthlyAssign.治療サブ)]; else if (sec === "RI") staff = [...split(monthlyAssign.RI), ...split(monthlyAssign.RIサブ)]; else if (monthlyAssign[sec] !== undefined) staff = split(monthlyAssign[sec]); return staff.map(extractStaffName); }
export function isMonthlyMainStaff(section: string, name: string, monthlyAssign: Record<string, string>) { if (!section) return false; return getMonthlyStaffForSection(section, monthlyAssign).includes(extractStaffName(name)); }
export function getStaffAmount(name: string) { if (ROLE_PLACEHOLDERS.includes(extractStaffName(name))) return 0; return (name.includes("(AM)") || name.includes("(PM)") || name.match(/\(〜/) || name.match(/〜\)/)) ? 0.5 : 1; }
...
const Modal = ({t, oC, w, ch}:any) => <div className="modal-overlay" onClick={oC}><div className={`modal-content ${w?'modal-wide':''}`} onClick={e=>e.stopPropagation()}><div className="modal-header"><h3 className="modal-title">{t}</h3><button onClick={oC} className="close-btn">✖</button></div>{ch}<div style={{textAlign:"center",marginTop:32}}><Btn bg="#2563eb" c="#fff" sz={16} oC={oC} ch="閉じる" style={{width:"100%", justifyContent:"center"}}/></div></div></div>;
const RuleCard = ({bg, b, c, ic, t, d, ch}:any) => <div style={{background:bg,padding:24,borderRadius:12,border:`2px solid ${b}`,marginBottom:20}}><h5 style={{margin:"0 0 12px 0",color:c,fontSize:18,fontWeight:800}}>{ic} {t}</h5>{d&&<p style={{fontSize:15,color:"#166534",marginTop:0,marginBottom:16}}>{d}</p>}{ch}</div>;
const MultiPicker = ({v, oC, opt, ph}:any) => { const c=split(v); return <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8,marginBottom:8}}>{c.map((x,i)=><div key={i} style={{background:"#e0f2fe",color:"#0369a1",borderRadius:16,padding:"6px 12px",fontSize:15,fontWeight:700,border:"1px solid #bae6fd",display:"flex",alignItems:"center",gap:6}}><span>{x}</span><span onClick={()=>{const n=[...c];n.splice(i,1);oC(join(n));}} style={{cursor:"pointer",opacity:0.5}}>✖</span></div>)}<select className="rule-sel" onChange={e=>{if(e.target.value&&!c.includes(e.target.value))oC(join([...c,e.target.value]));e.target.value="";}} value=""><option value="">{ph||"＋追加"}</option>{opt.filter((s:any)=>!c.includes(s)).map((s:any)=><option key={s} value={s}>{s}</option>)}</select></div>; };
const DelBtn = ({oC}:any) => <button onClick={oC} className="rule-del">✖</button>;
const NumInp = ({v, oC, w=60}:any) => <input type="number" value={v} onChange={e=>oC(Number(e.target.value))} className="rule-num" style={{width:w}} />;
...
const renderLog = (ls: string, i: number) => {
...
export class AutoAssigner {
  ...
  gTR(s: string) { let c=0; Object.keys(this.dc).forEach(k=>{if(REST_SECTIONS.includes(k)||["待機","昼当番","受付","受付ヘルプ"].includes(k))return; split(this.dc[k]).forEach(m=>{if(extractStaffName(m)===s&&!m.includes("17:")&&!m.includes("18:")&&!m.includes("19:")&&!m.includes("22:"))c++;});}); return c; }
  gTC(s: string) { let a=0,p=0; Object.keys(this.dc).forEach(k=>{if(["待機","昼当番","受付","受付ヘルプ"].includes(k)||REST_SECTIONS.includes(k))return; split(this.dc[k]).forEach(m=>{if(extractStaffName(m)===s){if(m.includes("(AM)"))a++;else if(m.includes("(PM)"))p++;else{a++;p++;}}});}); return {a,p}; }
  ...
  cAK(st: string, tgt: string, bp=false) { if(!split(this.dc[tgt]).map(extractStaffName).includes(st) && this.gTR(st)>=(this.cx.customRules.alertMaxKenmu||3)) return false; const exP=(this.cx.customRules.kenmuPairs||[]).filter((p:any)=>p.isExclusive); for(const p of exP){ const i1=split(this.dc[p.s1]).map(extractStaffName).includes(st); const i2=split(this.dc[p.s2]).map(extractStaffName).includes(st); if((i1||i2)&&tgt!==p.s1&&tgt!==p.s2)return false; if((tgt===p.s1||tgt===p.s2)&&!bp){if(ROOM_SECTIONS.filter(r=>split(this.dc[r]).map(extractStaffName).includes(st)&&!["待機", "昼当番", "受付", "受付ヘルプ"].includes(r)).some(r=>r!==p.s1&&r!==p.s2))return false;} } return true; }
  ...
  initCounts() { this.cx.allStaff.forEach((s: string)=>{this.ac[s]=0;this.ma[s]=1;this.rc[s]={};SECTIONS.forEach(x=>this.rc[s][x]=0);this.cts[s]=0;}); this.pastDaysInMonth.forEach(pd=>Object.entries(pd.cells).forEach(([k,v])=>{if(["CT","MRI"].includes(k))split(v).forEach(m=>{const c=extractStaffName(m);if(this.rc[c]){this.rc[c][k]++;this.cts[c]++;}});})); this.pastDaysInWeek.forEach(pd=>Object.entries(pd.cells).forEach(([k,v])=>{if(!["CT","MRI"].includes(k))split(v).forEach(m=>{const c=extractStaffName(m);if(this.rc[c]){this.rc[c][k]++;this.cts[c]++;}});})); }
  buildBlockMap() { this.cx.allStaff.forEach((s: string)=>this.bm.set(s,'NONE')); ["明け","入り","土日休日代休"].forEach(s=>split(this.dc[s]).forEach(m=>this.bm.set(extractStaffName(m),'ALL'))); split(this.dc["不在"]).forEach(m=>this.bm.set(extractStaffName(m),m.includes("(AM)")?'AM':m.includes("(PM)")?'PM':'ALL')); }
  applyDailyAdditions() { (this.cx.customRules.dailyAdditions||[]).forEach((r:any)=>{if(r.date===this.d.id&&r.section&&r.count>0&&r.section!=="透析後胸部"){const p=r.section+"枠"+(r.time==="全日"||!r.time?"":r.time); let c=split(this.dc[r.section]); if(!c.includes(p)){for(let i=0;i<r.count;i++)c.push(p);this.dc[r.section]=join(c);}}}); }
  evaluateEmergencies() { const tAv=this.cx.activeGeneralStaff.filter((s: string)=>this.bm.get(s)!=='ALL').length; (this.cx.customRules.emergencies||[]).forEach((em:any)=>{if(tAv<=Number(em.threshold)){if(em.type==="role_assign"&&em.role&&em.section)this.ra[em.role]=em; if(em.type==="staff_assign"&&em.staff&&em.section)this.stA.push(em); if(em.type==="clear"&&em.section){this.sk.push(em.section);this.cl.push(em.section);} if(em.type==="change_capacity"&&em.section)this.cap[em.section]=Number(em.newCapacity);}}); }
  cleanUpDayCells() { Object.keys(this.dc).forEach(s=>{if(["明け","入り","不在","土日休日代休"].includes(s))return; if(this.sk.includes(s)){this.dc[s]="";return;} this.dc[s]=join(split(this.dc[s]).map(m=>{const c=extractStaffName(m);if(ROLE_PLACEHOLDERS.includes(c))return m; const b=this.bm.get(c); if(b==='ALL'||(b==='AM'&&m.includes('(AM)'))||(b==='PM'&&m.includes('(PM)')))return null; if(b==='AM'&&!m.includes('(PM)')&&!m.match(/\(.*\)/))return `${c}(PM)`; if(b==='PM'&&!m.includes('(AM)')&&!m.match(/\(.*\)/))return `${c}(AM)`; return m;}).filter(Boolean) as string[]); }); }
  prepareAvailability() { const sSL=split(this.cx.customRules.supportStaffList||"").map(extractStaffName); this.iaAll=this.cx.allStaff.filter((s: string)=>this.bm.get(s)!=='ALL').sort((a: string, b: string)=>(this.cts[a]||0)-(this.cts[b]||0)||a.localeCompare(b,'ja')); this.iaSup=this.iaAll.filter(s=>sSL.includes(s)); this.iaGen=this.iaAll.filter(s=>this.cx.activeGeneralStaff.includes(s)&&!sSL.includes(s)); this.iaRec=this.iaAll.filter(s=>this.cx.activeReceptionStaff.includes(s)||(this.cx.activeGeneralStaff.includes(s)&&!sSL.includes(s))); }
      ...
      priority.forEach((room: string) => {
         ...
         while (getAmt(current) < eff.cap) {
            const freeStaff = sortedAvail.find((s: string) => { 
                ...
                const b = this.blockMap.get(s); if ((eff.pC && b === 'AM') || (eff.aC && b === 'PM') || this.noHD(room, b || "")) return false; return true; 
            });
            if (!freeStaff) break;
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number) {
    if (this.skipSections.includes(section) || section === "透析後胸部") return; const eff = this.gET(section, targetCount); if (eff.al) return;
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      current.push(`${c}${t}`); this.addU(c, f); this.log(`✅ [配置決定] ${section} に ${c}${t} ${fM}`);
    }
  ...
  processPostTasks() {
    ...
    this.iaSup.forEach(staff => { if (this.isUsed(staff)) return; let asg = false; for (const rm of split(this.ctx.customRules.supportTargetRooms)) { if (this.skipSections.includes(rm) || this.isForbidden(staff, rm) || rm === "透析後胸部") continue; let c = split(this.dayCells[rm]); if (c.length > 0 && !c.map(extractStaffName).includes(staff) && !this.hasNGPair(staff, c.map(extractStaffName), false) && !this.iHNC(staff, rm)) { const b = this.blockMap.get(staff); let t = b === 'AM' ? "(PM)" : b === 'PM' ? "(AM)" : ""; if (this.noHD(rm, t)) continue; this.dayCells[rm] = join([...c, `${staff}${t}`]); this.addU(staff, t ? 0.5 : 1); this.blockMap.set(staff, 'ALL'); asg = true; break; } } if (!asg) { for (const rm of split(this.ctx.customRules.supportTargetRooms)) { if (this.skipSections.includes(rm) || this.isForbidden(staff, rm) || rm === "透析後胸部") continue; if (!split(this.dayCells[rm]).length && !this.iHNC(staff, rm) && this.canAddKenmu(staff, rm)) { const b = this.blockMap.get(staff); let t = b === 'AM' ? "(PM)" : b === 'PM' ? "(AM)" : ""; if (this.noHD(rm, t)) continue; this.dayCells[rm] = `${staff}${t}`; this.addU(staff, t ? 0.5 : 1); this.blockMap.set(staff, 'ALL'); break; } } } });
    ...
    (this.ctx.customRules.swapRules || []).forEach((r: any) => {
      ...
      if (!tM.some(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, r.targetRoom) && !this.iHNC(c, r.targetRoom) && !this.iHB(c, r.targetRoom).h && this.canAddKenmu(c, r.targetRoom, true); })) {
        let sw = false;
        for (const sSR of split(r.sourceRooms).sort((a, b) => this.gRSc(parseRoomCond(a).r, r.targetRoom) - this.gRSc(parseRoomCond(b).r, r.targetRoom))) {
          ...
          let sC = sM.filter(m => !ROLE_PLACEHOLDERS.includes(extractStaffName(m)) && !this.isForbidden(extractStaffName(m), r.targetRoom) && !this.iHB(extractStaffName(m), r.targetRoom).h && !this.iHNC(extractStaffName(m), r.targetRoom) && this.canAddKenmu(extractStaffName(m), r.targetRoom, true) && !this.isForbidden(extractStaffName(m), r.triggerRoom) && !this.noHD(r.targetRoom, m));
          ...
          for (const sm of sC) { const sCo = extractStaffName(sm); const tTK = tM.find(m => !this.isForbidden(extractStaffName(m), sR) && !this.iHB(extractStaffName(m), sR).h && !this.hasNGPair(extractStaffName(m), sM.map(extractStaffName), false) && this.canAddKenmu(extractStaffName(m), sR, true)); if (tTK && this.canAddKenmu(sCo, r.targetRoom, true)) { this.dayCells[r.triggerRoom] = join(tM.map(m => m === tTK ? m.replace(extractStaffName(tTK), sCo) : m)); this.dayCells[sR] = join(sM.map(m => m === sm ? m.replace(sCo, extractStaffName(tTK)) : m)); this.log(`🔄 [玉突き] ${r.triggerRoom}の${extractStaffName(tTK)}と ${sR}の${sCo}を交換`); sw = true; break; } } if (sw) break;
        }
      }
    });

    let uG1 = this.iaGen.filter(s => !this.isUsed(s) && this.blockMap.get(s) !== 'ALL');
    (this.ctx.customRules.linkedRooms || []).forEach((r: any) => {
      if (!r.target || this.skipSections.includes(r.target) || r.target === "透析後胸部") return; const tC = this.dynamicCapacity[r.target] ?? (["CT", "MRI", "治療"].includes(r.target) ? 3 : 1); const e = this.gET(r.target, tC); if (e.al) return;
      ...
      for (const sSR of split(r.sources)) { if (cA >= tC && cP >= tC) break; const { r: sR, min } = parseRoomCond(sSR); if (sR === "透析後胸部" || (min > 0 && split(this.dayCells[sR]).reduce((s, m) => s + getStaffAmount(m), 0) < min)) continue;
        split(this.dayCells[sR]).forEach(m => { if (cA >= tC && cP >= tC) return; const c = extractStaffName(m); if (!ROLE_PLACEHOLDERS.includes(c) && !cM.map(extractStaffName).includes(c) && !this.isForbidden(c, r.target) && !this.iHB(c, r.target).h && !this.hasNGPair(c, cM.map(extractStaffName), false) && !this.iHNC(c, r.target) && (r.target === "MMG" ? this.isMmgCapable(c) : true) && this.canAddKenmu(c, r.target, true) && !m.includes("17:") && !m.includes("19:") && !this.noHD(r.target, m)) { let pS = m; if (r.target === "パノラマCT" && sR === "透視（6号）") { if (m.includes("(PM)")) return; pS = `${c}(AM)`; } else { if (cA < tC && cP >= tC) { if (m.includes("(PM)")) return; pS = `${c}(AM)`; } else if (cA >= tC && cP < tC) { if (m.includes("(AM)")) return; pS = `${c}(PM)`; } else if (e.pC) { if (m.includes("(PM)")) return; pS = `${c}(AM)`; } else if (e.aC) { if (m.includes("(AM)")) return; pS = `${c}(PM)`; } } cM.push(pS); if (pS.includes("(AM)")) cA++; else if (pS.includes("(PM)")) cP++; else { cA++; cP++; } this.addU(c, getStaffAmount(pS)); this.updateBlockMapAfterKenmu(c, pS); this.log(`🔗 [基本兼務] ${sR} の ${pS} を ${r.target} にセット配置しました`); } });
      } this.dayCells[r.target] = join(cM);
    });

    ROOM_SECTIONS.forEach(tR => {
      if (this.clearSections.includes(tR) || ["待機", "昼当番", "受付", "受付ヘルプ", "透析後胸部"].includes(tR)) return; const tC = this.dynamicCapacity[tR] ?? (["CT", "MRI", "治療"].includes(tR) ? 3 : 1); const e = this.gET(tR, tC); if (e.al) return;
      let cM = split(this.dayCells[tR]); let cA = e.aC ? 999 : 0, cP = e.pC ? 999 : 0; cM.forEach(x => { if (x.includes("(AM)")) cA++; else if (x.includes("(PM)")) cP++; else { cA++; cP++; } }); if (cM.length > 0 && cM.every(m => sSL.includes(extractStaffName(m)))) { cA = e.aC ? 999 : 0; cP = e.pC ? 999 : 0; } if (cA >= tC && cP >= tC) return;
      const mR = (this.ctx.customRules.rescueRules || []).filter((r: any) => r.targetRoom === tR); let sRms = mR.length > 0 ? mR.flatMap((r: any) => split(r.sourceRooms || "")).sort((a: string, b: string) => this.gRSc(parseRoomCond(a).r, tR) - this.gRSc(parseRoomCond(b).r, tR)) : [...split(this.ctx.customRules.supportTargetRoomsLowImpact || "3号室,パノラマCT"), "2号室", "1号室", "5号室", "CT(4)"].filter(r => r !== tR);
      if (sRms.length > 0) { let cnds: { c: string, fS: string, i: number }[] = []; sRms.forEach((sS: string, i: number) => { const { r: sR, min } = parseRoomCond(sS); if (sR === tR || sR === "透析後胸部" || (min > 0 && split(this.dayCells[sR]).reduce((s, m) => s + getStaffAmount(m), 0) < min)) return; split(this.dayCells[sR]).forEach(m => { const c = extractStaffName(m); if (!ROLE_PLACEHOLDERS.includes(c) && !this.iHNC(c, tR) && !cnds.some(x => x.c === c) && !this.isForbidden(c, tR) && !this.iHB(c, tR).h && !m.includes("17:") && !this.noHD(tR, m)) cnds.push({ c, fS: m, i }); }); }); const cCs = cM.map(extractStaffName); cnds = cnds.filter(c => !cCs.includes(c.c) && (tR === "MMG" ? this.isMmgCapable(c.c) : true) && this.canAddKenmu(c.c, tR, true)); cnds.sort((a, b) => this.getTodayRoomCount(a.c) - this.getTodayRoomCount(b.c) || this.getPastRoomCount(a.c, tR) - this.getPastRoomCount(b.c, tR) || a.i - b.i || (this.assignCounts[a.c] || 0) - (this.assignCounts[b.c] || 0)); for (const cn of cnds) { if (cA >= tC && cP >= tC) break; if (this.hasNGPair(cn.c, cCs, false)) continue; let pS = cn.fS; if (cA < tC && cP >= tC) { if (cn.fS.includes("(PM)")) continue; pS = `${cn.c}(AM)`; } else if (cA >= tC && cP < tC) { if (cn.fS.includes("(AM)")) continue; pS = `${cn.c}(PM)`; } else if (e.pC) { if (cn.fS.includes("(PM)")) continue; pS = `${cn.c}(AM)`; } else if (e.aC) { if (cn.fS.includes("(AM)")) continue; pS = `${cn.c}(PM)`; } cM.push(pS); if (pS.includes("(AM)")) cA++; else if (pS.includes("(PM)")) cP++; else { cA++; cP++; } this.addU(cn.c, getStaffAmount(pS)); this.updateBlockMapAfterKenmu(cn.c, pS); } this.dayCells[tR] = join(cM); }
    });
    ...
    (this.ctx.customRules.emergencies || []).forEach((em: any) => {
      ...
      let sw = false; for (const sF of sRL) { if (sw || sF === "透析後胸部") break; const sM = split(this.dayCells[sF]); if (!sM.length) continue; const ngI = sM.filter(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && (this.isForbidden(c, wR) || !this.canAddKenmu(c, wR, true)); }); if (!ngI.length) continue; for (const src of ROOM_SECTIONS.filter(r => r !== wR && r !== sF && !this.skipSections.includes(r) && !["待機", "昼当番", "受付", "受付ヘルプ", "透析後胸部"].includes(r))) { if (sw) break; const rM = split(this.dayCells[src]); const oC = rM.filter(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, wR) && !this.isForbidden(c, sF) && !this.iHB(c, wR).h && !this.iHNC(c, wR) && this.canAddKenmu(c, wR, true) && !m.includes("17:") && !m.includes("19:") && !this.noHD(wR, m); }); for (const om of oC) { const oc = extractStaffName(om); const km = ngI.find(m => { const c = extractStaffName(m); return !this.isForbidden(c, src) && !this.iHB(c, src).h && !this.hasNGPair(c, rM.map(extractStaffName), false) && this.canAddKenmu(c, src, true); }); if (!km) continue; const kc = extractStaffName(km); this.dayCells[sF] = join(sM.map(m => m === km ? m.replace(kc, oc) : m)); this.dayCells[src] = join(rM.map(m => m === om ? m.replace(oc, kc) : m)); sw = true; break; } } }
    });
    ...
    if (r2A < 2 && (nAH || nPH)) {
      const fC = (iA: boolean, exS: string[]) => [...sSL, ...this.initialAvailGeneral].filter((s, i, a) => a.indexOf(s) === i).find(s => { if (r2C.includes(s) || this.isForbidden(s, "2号室") || this.iHB(s, "2号室").h || this.hasNGPair(s, r2C, false) || !this.canAddKenmu(s, "2号室") || this.isHardNoConsecutive(s, "2号室")) return false; const b = this.blockMap.get(s); if (b === 'ALL' || (iA && b === 'AM') || (!iA && b === 'PM')) return false; if (exS.some(r => split(this.dayCells[r]).map(extractStaffName).includes(s))) return false; return true; });
      if (nAH) { let p = fC(true, ["1号室", "5号室"]) ?? fC(true, []); if (p) { this.dayCells["2号室"] = join([...split(this.dayCells["2号室"]), `${p}(AM)`]); this.addU(p, 0.5); this.blockMap.set(p, this.blockMap.get(p) === 'PM' ? 'ALL' : 'AM'); this.log(`🤝 [ポータブル特例] 2号室のポータブル兼務(AM)のため ${p} を追加`); } }
      if (nPH) { let p = fC(false, ["1号室", "5号室"]) ?? fC(false, []); if (p) { this.dayCells["2号室"] = join([...split(this.dayCells["2号室"]), `${p}(PM)`]); this.addU(p, 0.5); this.blockMap.set(p, this.blockMap.get(p) === 'AM' ? 'ALL' : 'PM'); this.log(`🤝 [ポータブル特例] 2号室のポータブル兼務(PM)のため ${p} を追加`); } }
    }
    ...
    uG2.forEach(st => {
      const b = this.blockMap.get(st); if (b === 'ALL') return; let t = b === 'AM' ? "(PM)" : b === 'PM' ? "(AM)" : ""; let asg = false;
      for (const rm of dKT) {
        if (this.skipSections.includes(rm) || this.isForbidden(st, rm) || (rm === "MMG" && !this.isMmgCapable(st))) continue; if (t !== "" && this.iHB(st, rm).h) continue; const e = this.gET(rm, 1); if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC)) continue;
        ...
        if (rI !== -1) {
          const oS = cM[rI], oC = extractStaffName(oS); let oR = ""; if (t === "(AM)" && !oS.includes("(")) oR = `${oC}(PM)`; else if (t === "(PM)" && !oS.includes("(")) oR = `${oC}(AM)`; cM[rI] = `${st}${t}`; if (oR) cM.push(oR); this.dayCells[rm] = join(cM); this.addU(st, t ? 0.5 : 1); this.blockMap.set(st, 'ALL'); this.assignCounts[oC] = Math.max(0, (this.assignCounts[oC] || 1) - getStaffAmount(oS)); let am = false, pm = false; ROOM_SECTIONS.forEach(r => { if (["待機", "昼当番", "受付", "受付ヘルプ"].includes(r)) return; split(this.dayCells[r]).forEach(m => { if (extractStaffName(m) === oC) { if (m.includes("(AM)")) am = true; else if (m.includes("(PM)")) pm = true; else { am = true; pm = true; } } }); }); if (am && pm) this.blockMap.set(oC, 'ALL'); else if (am) this.blockMap.set(oC, 'AM'); else if (pm) this.blockMap.set(oC, 'PM'); else this.blockMap.set(oC, 'NONE'); this.log(`🪄 [兼務解消] ${st} を専任化し ${oC} の負担軽減`); asg = true; break;
        }
      }
      if (!asg) { for (const rm of rP) { if (this.skipSections.includes(rm) || this.isForbidden(st, rm) || (rm === "MMG" && !this.isMmgCapable(st)) || ["待機", "昼当番", "受付", "受付ヘルプ", "CT", "MRI", "治療", "RI", "透析後胸部"].includes(rm) || (t !== "" && this.iHB(st, rm).h)) continue; const aC = this.dynamicCapacity[rm] ?? (["CT", "MRI", "治療"].includes(rm) ? 3 : 1); const e = this.gET(rm, aC); if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC) || !this.canAddKenmu(st, rm) || this.isHardNoConsecutive(st, rm) || this.noHD(rm, t)) continue; let cM = split(this.dayCells[rm]); if (cM.reduce((s, m) => s + getStaffAmount(m), 0) >= e.cap || this.hasNGPair(st, cM.map(extractStaffName), false)) continue; this.dayCells[rm] = join([...cM, `${st}${t}`]); this.addU(st, t ? 0.5 : 1); this.blockMap.set(st, 'ALL'); this.log(`♻️ [余剰配置] 余力のある ${st} を ${rm} に追加配置`); asg = true; break; } }
      if (!asg) { for (const fbR of ["3号室", "2号室", "1号室", "5号室"]) { if (this.skipSections.includes(fbR) || this.isForbidden(st, fbR)) continue; const e = this.gET(fbR, 1); if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC) || this.isHardNoConsecutive(st, fbR) || this.noHD(fbR, t)) continue; let cM = split(this.dayCells[fbR]); if (this.hasNGPair(st, cM.map(extractStaffName), false)) continue; this.dayCells[fbR] = join([...cM, `${st}${t}`]); this.addU(st, t ? 0.5 : 1); this.blockMap.set(st, 'ALL'); this.log(`🚨 [最終救済] 定員超過でも未配置を防ぐため ${st} を ${fbR} に強制配置`); asg = true; break; } }
    });
  ...
  return (
    <div style={{ maxWidth: "98%", margin: "0 auto", padding: "24px", boxSizing: "border-box" }}>
      ...
      <div className="no-print" style={{ ...panelStyle(), display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, padding: "20px 32px", background: "linear-gradient(to right, #ffffff, #f8fafc)" }}>
        <h2 style={{ margin: 0, color: "#0f172a", fontSize: 26, fontWeight: 900 }}>勤務割付システム Ver 2.94</h2>
      ...
      <div className="no-print" style={{ display: activeTab === 'rules' ? 'block' : 'none' }}>
        ...
        <div style={{ ...panelStyle() }}>
          ...
          <div style={{ borderLeft: "8px solid #94a3b8", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#f8fafc" b="#cbd5e1" c="#334155" ic="🙅" t="担当不可ルール" ch={
              <>{(customRules.forbidden || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 16, borderBottom: "1px solid #e2e8f0", paddingBottom: 16 }}>
                    ...
                    <MultiPicker v={rule.sections} oC={(v: string) => updateRule("forbidden", idx, "sections", v)} opt={ASSIGNABLE_SECTIONS} />
                  </div>
              ))}<button className="rule-add" style={{color:"#475569", borderColor:"#cbd5e1"}} onClick={() => addRule("forbidden", { staff: "", sections: "" })}>＋ 追加</button></>
            } />

            <RuleCard bg="#f0fdf4" b="#bbf7d0" c="#15803d" ic="🔒" t="専従ルール" ch={
            ...
            <RuleCard bg="#fef2f2" b="#fecaca" c="#b91c1c" ic="🚫" t="NGペア" ch={
            ...
            <RuleCard bg="#f8fafc" b="#cbd5e1" c="#334155" ic="🕒" t="終日専任・連日禁止" ch={
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 260 }}><label style={{ fontSize: 17, fontWeight: 800, color: "#475569", display: "block", marginBottom: 8 }}>【終日専任】半休・AM/PM不可の部屋</label><MultiPicker v={customRules.fullDayOnlyRooms ?? ""} oC={(v: string) => setCustomRules({...customRules, fullDayOnlyRooms: v})} opt={ROOM_SECTIONS} /></div>
                <div style={{ flex: 1, minWidth: 260 }}><label style={{ fontSize: 17, fontWeight: 800, color: "#475569", display: "block", marginBottom: 8 }}>【連日禁止】2日連続で担当させない部屋</label><MultiPicker v={customRules.noConsecutiveRooms ?? ""} oC={(v: string) => setCustomRules({...customRules, noConsecutiveRooms: v})} opt={ROOM_SECTIONS} /></div>
              </div>
            } />

            <RuleCard bg="#fdf4ff" b="#f5d0fe" c="#86198f" ic="🏠" t="遅番不可スタッフ・部屋" ch={
              <><label style={{ fontSize: 17, fontWeight: 800, color: "#86198f", display: "block", marginBottom: 8 }}>遅番不可スタッフ</label>
              <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "2px solid #f0abfc", minHeight: "56px", display: "flex", alignItems: "center", marginBottom: 16 }}><MultiPicker v={customRules.noLateShiftStaff || ""} oC={(v: string) => setCustomRules({...customRules, noLateShiftStaff: v})} opt={allStaff} ph="＋スタッフを選択" /></div>
              <label style={{ fontSize: 17, fontWeight: 800, color: "#86198f", display: "block", marginBottom: 8 }}>その日その部屋に入っている人も遅番除外</label>
              <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "2px solid #f0abfc", minHeight: "56px", display: "flex", alignItems: "center" }}><MultiPicker v={customRules.noLateShiftRooms || ""} oC={(v: string) => setCustomRules({...customRules, noLateShiftRooms: v})} opt={ROOM_SECTIONS} ph="＋部屋を選択" /></div></>
            } />
          </div>
          ...
          <div style={{ borderLeft: "8px solid #f59e0b", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fef08a" b="#fde047" c="#a16207" ic="🚨" t="緊急ルール（人数不足時）" ch={
            ...
            <RuleCard bg="#f0fdf4" b="#bbf7d0" c="#15803d" ic="🔄" t="メイン配置の交換ルール" ch={
              <>{(customRules.swapRules || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 20, borderBottom: "2px solid #bbf7d0", paddingBottom: 20 }}>
                    ...
                    <div className="rule-row">
                      ...
                      <MultiPicker v={rule.sourceRooms} oC={(v: string) => updateRule("swapRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} />
                    </div>
                  </div>
            ...
            <RuleCard bg="#fff7ed" b="#fed7aa" c="#c2410c" ic="🔄" t="代打ルール" ch={
              <>{(customRules.substitutes || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 16, alignItems: "center", background: "#fff", padding: "16px", borderRadius: 8, border: "1px solid #fdba74", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
                    <div style={{ flex: 1, minWidth: "200px" }}><MultiPicker v={rule.target} oC={(v: string) => updateRule("substitutes", idx, "target", v)} opt={activeGeneralStaff} ph="対象スタッフ(休)" /></div>
                    <span style={{ fontSize: 17, fontWeight: 800, color: "#c2410c" }}>が全員休みの時➔</span>
                    <div style={{ flex: 1, minWidth: "200px" }}><MultiPicker v={rule.subs} oC={(v: string) => updateRule("substitutes", idx, "subs", v)} opt={activeGeneralStaff} ph="代打スタッフを追加" /></div>
            ...
            <RuleCard bg="#e0f2fe" b="#bae6fd" c="#0369a1" ic="🎱" t="玉突きルール" ch={
              <>{(customRules.pushOuts || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 20, borderBottom: "1px solid #bae6fd", paddingBottom: 20 }}>
                    ...
                    <div className="rule-row">
                      ...
                      <MultiPicker v={rule.targetSections} oC={(v: string) => updateRule("pushOuts", idx, "targetSections", v)} opt={ROOM_SECTIONS} />
                    </div>
                  </div>
          ...
          <div style={{ borderLeft: "8px solid #3b82f6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fffbeb" b="#fde68a" c="#b45309" ic="👑" t="部屋の割り当て優先順位" ch={
            ...
            <RuleCard bg="#f8fafc" b="#cbd5e1" c="#334155" ic="👥" t="絶対優先の定員設定" ch={
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            <RuleCard bg="#fff" b="#e2e8f0" c="#334155" ic="📅" t="月間担当者の設定" ch={
          ...
          <div style={{ borderLeft: "8px solid #10b981", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fdf4ff" b="#f0abfc" c="#86198f" ic="✨" t="スマート兼務（専任担当の負担軽減・引き抜き）" ch={
              <>{(customRules.smartKenmu || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ background: "#fff", padding: "16px 20px", border: "1px solid #f0abfc", borderRadius: 8, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 6 }}><MultiPicker v={rule.sourceRooms} oC={(v: string) => updateRule("smartKenmu", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
            ...
            <RuleCard bg="#ecfdf5" b="#a7f3d0" c="#065f46" ic="🔗" t="兼務・セット配置ルール" ch={
              ...
              {(customRules.linkedRooms || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #a7f3d0", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 10 }}><MultiPicker v={rule.sources} oC={(v: string) => updateRule("linkedRooms", idx, "sources", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
              ...
              {(customRules.rescueRules || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #fde047", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10 }}><MultiPicker v={rule.sourceRooms} oC={(v: string) => updateRule("rescueRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flexShrink: 0, alignItems: "center" }}>
            ...
            <RuleCard bg="#f5f3ff" b="#ddd6fe" c="#6d28d9" ic="🌆" t="遅番ルール" ch={
              <><div style={{ marginBottom: 16, background: "#fff", padding: 16, borderRadius: 8, border: "1px solid #ddd6fe" }}>
                ...
                <MultiPicker v={customRules.lateShiftLowPriorityStaff || ""} oC={(v: string) => setCustomRules({...customRules, lateShiftLowPriorityStaff: v})} opt={allStaff} ph="＋スタッフを選択" />
            ...
            <RuleCard bg="#fff1f2" b="#fecaca" c="#be185d" ic="⚠️" t="兼務上限のストッパー設定" ch={
          ...
          <div style={{ borderLeft: "8px solid #8b5cf6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#eef2ff" b="#c7d2fe" c="#4338ca" ic="🍱" t="昼当番ルール" ch={
              ...
              <div style={{ background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", marginBottom: 20 }}>
                ...
                {(customRules.lunchRoleRules || []).map((rule: any, idx: number) => (
                  <div key={idx} className="rule-row" style={{ borderBottom: "1px dashed #e0e7ff", paddingBottom: 16, marginBottom: 16 }}>
                    ...
                    <MultiPicker v={rule.sourceRooms} oC={(v: string) => updateRule("lunchRoleRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} />
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 20, marginTop: 24 }}>
                  <div style={{ flex: 1, background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", minWidth: "260px" }}>
                    ...
                    <MultiPicker v={customRules.lunchPrioritySections ?? "RI,1号室,2号室,3号室,5号室,CT"} oC={(v: string) => setCustomRules({...customRules, lunchPrioritySections: v})} opt={ROOM_SECTIONS} />
                  </div>
                  <div style={{ flex: 1, background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", minWidth: "260px" }}>
                    <h6 style={{ margin: "0 0 12px 0", fontSize: 17, color: "#4f46e5", fontWeight: 800 }}>⚠️ 緊急時のみ選出する部屋（なるべく除外）</h6>
                    <MultiPicker v={customRules.lunchLastResortSections ?? "治療"} oC={(v: string) => setCustomRules({...customRules, lunchLastResortSections: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div></>
            } />

            <RuleCard bg="#f0fdf4" b="#bbf7d0" c="#15803d" ic="🤝" t="サポート専任（2人目要員）ルール" ch={
              <><div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: "260px" }}>
                    ...
                    <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "1px solid #86efac", minHeight: "56px", display: "flex", alignItems: "center" }}>
                      <MultiPicker v={customRules.supportStaffList || ""} oC={(v: string) => setCustomRules({...customRules, supportStaffList: v})} opt={allStaff} ph="＋スタッフを選択" />
                    </div>
                  </div>
                  <div style={{ flex: 2, minWidth: "260px" }}>
                    <label style={{ fontSize: 17, fontWeight: 800, color: "#166534", display: "block", marginBottom: 8 }}>優先する対象部屋</label>
                    <MultiPicker v={customRules.supportTargetRooms ?? "1号室,2号室,5号室,パノラマCT"} oC={(v: string) => setCustomRules({...customRules, supportTargetRooms: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div>
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start", marginTop: 16 }}>
                  <div style={{ flex: 1, minWidth: "260px" }}>
                    ...
                    <MultiPicker v={customRules.supportTargetRoomsLowImpact ?? "3号室,パノラマCT"} oC={(v: string) => setCustomRules({...customRules, supportTargetRoomsLowImpact: v})} opt={ROOM_SECTIONS} />
                  </div>
                  <div style={{ flex: 2, minWidth: "260px" }}>
                    <label style={{ fontSize: 15, fontWeight: 800, color: "#166534", display: "block", marginBottom: 4 }}>高影響グループ（極力補充元にしない部屋）</label>
                    <MultiPicker v={customRules.supportTargetRoomsHighImpact ?? "CT,MRI,治療,RI,ポータブル,2号室,1号室,5号室,透視（6号）,透視（11号）,骨塩,検像"} oC={(v: string) => setCustomRules({...customRules, supportTargetRoomsHighImpact: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div></>
            } />
          </div>
        ...
        <Modal t="🏥 勤務割付システムのルールブック" oC={() => setShowRuleModal(false)} wide ch={
          <div style={{ lineHeight: 1.8, fontSize: "16px", color: "#334155" }}>
            ...
            <ol style={{ paddingLeft: 24, marginBottom: 24 }}>
              ...
              <li style={{ marginBottom: 8 }}><strong>総仕上げ（昼当番・余剰配置）:</strong> 昼当番を決定後、余力のあるスタッフは優先的に兼務解消（専任化）にあてられます。また未配置の人をなくすため、最後の余
