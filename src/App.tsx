...
export class AutoAssigner {
  ...
  originalDayCells: Record<string, string>;
  ...
  private getAbsenceHelpTargetRoom(staff: string): string | null {
    ...
    for (const room of ROOM_SECTIONS) {
      if (split(this.originalDayCells[room] || "").some(m => extractStaffName(m) === core)) return room;
    }
    return null;
  }
  private getOriginalAbsenceEntry(staff: string): string | null {
    return split(this.dayCells["不在"] || this.originalDayCells["不在"] || "").find(m => extractStaffName(m) === extractStaffName(staff)) || null;
  }
  private getAbsenceTag(entry: string | null): string {
    if (!entry) return "";
    const core = extractStaffName(entry);
    return entry.substring(core.length);
  }
  private getNoHelpTagsForRoom(room: string): string[] {
    const helpMap = parseAbsenceHelpMap(this.dayCells);
    const tags: string[] = [];
    for (const [staff, helpTag] of Object.entries(helpMap)) {
      if (helpTag !== ABSENCE_HELP_NONE) continue;
      const targetRoom = this.getAbsenceHelpTargetRoom(staff);
      if (targetRoom !== room) continue;
      const absenceEntry = this.getOriginalAbsenceEntry(staff);
      const absenceTag = this.getAbsenceTag(absenceEntry);
      if (absenceTag) tags.push(absenceTag);
    }
    return tags;
  }
  private enforceSupportAssignments() {
    const supportStaff = split(this.ctx.customRules.supportStaffList || "").map(extractStaffName);
    const targets = split(this.ctx.customRules.supportTargetRooms || "").map((x: string) => parseRoomCond(x).r).filter(Boolean);
    for (const staff of supportStaff) {
      if (!this.initialAvailAll.includes(staff)) continue;
      if (targets.some(room => split(this.dayCells[room] || "").map(extractStaffName).includes(staff))) continue;
      if (this.blockMap.get(staff) === 'ALL') continue;
      for (const room of targets) {
        if (this.skipSections.includes(room) || room === "透析後胸部" || this.isForbidden(staff, room) || this.isHardNoConsecutive(staff, room) || this.isHalfDayBlocked(staff, room).hard) continue;
        const current = split(this.dayCells[room] || "");
        if (this.hasNGPair(staff, current.map(extractStaffName), false) || !this.canAddKenmu(staff, room, true)) continue;
        ...
        this.blockMap.set(staff, 'ALL');
        this.log(`🧷 [サポート確定] ${staff} を ${room} に配置`);
        break;
      }
    }
  }
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number, forcedNeedTag: string = "") {
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const noHelpTags = this.getNoHelpTagsForRoom(section);
      if (needTag && noHelpTags.some(tag => tagsOverlap(tag, needTag))) break;
      const getFilterReason = (name: string): RejectReason | null => { if (current.map(extractStaffName).includes(name)) return { hard: true, msg: "同室配置済" }; if (this.isUsed(name)) return { hard: true, msg: "他業務配置済" }; if (this.isForbidden(name, section)) return { hard: true, msg: "担当不可" }; if (section === "MMG" && !this.isMmgCapable(name)) return { hard: true, msg: "MMG外" }; if (!this.canAddKenmu(name, section)) return { hard: true, msg: "兼務上限" }; const b = this.blockMap.get(name); if (b === 'ALL') return { hard: true, msg: "全日ブ" }; if (needTag && !this.canStaffCoverNeedTag(name, needTag)) return { hard: true, msg: "時間不一致" }; if (needTag && b === 'NONE' && ["CT", "MRI", "治療", "RI"].includes(section)) { if (!eff.pmClosed && !eff.amClosed && !isMonthlyMainStaff(section, name, this.ctx.monthlyAssign)) return { hard: true, msg: "半端枠" }; } if (eff.pmClosed && b === 'AM') return { hard: true, msg: "午後休" }; if (eff.amClosed && b === 'PM') return { hard: true, msg: "午前休" }; if (this.isTimeTagBlockedByFullDayRule(section, needTag || this.getAssignedWorkTag(name) || "")) return { hard: true, msg: "終日専任" }; if (this.isHalfDayBlocked(name, section).hard) return { hard: true, msg: "終日専任" }; if (this.isHardNoConsecutive(name, section)) return { hard: false, msg: "連日禁止" }; if (this.hasNGPair(name, current.map(extractStaffName), false)) return { hard: true, msg: "絶対NG" }; if (this.hasNGPair(name, current.map(extractStaffName), true)) return { hard: false, msg: "なるべくNG" }; return null; };
  ...
  processPostTasks() {
    ...
    const roomNeedsFlexibleCoverage = (room: string): boolean => {
      const asSourceOrTargetInKenmu = (this.ctx.customRules.kenmuPairs || []).some((p: any) => p.s1 === room || p.s2 === room);
      const asSourceOrTargetInLinked = (this.ctx.customRules.linkedRooms || []).some((r: any) =>
        r.target === room || split(r.sources || "").some((x: string) => parseRoomCond(x).r === room)
      );
      const asSourceOrTargetInRescue = (this.ctx.customRules.rescueRules || []).some((r: any) =>
        r.targetRoom === room || split(r.sourceRooms || "").some((x: string) => parseRoomCond(x).r === room)
      );
      const supportTargets = split(this.ctx.customRules.supportTargetRooms || "").map((x: string) => parseRoomCond(x).r);
      const supportLowImpact = split(this.ctx.customRules.supportTargetRoomsLowImpact || "").map((x: string) => parseRoomCond(x).r);
      const portableRelated = room === "2号室" || room === "ポータブル" || room === "パノラマCT";
      return asSourceOrTargetInKenmu || asSourceOrTargetInLinked || asSourceOrTargetInRescue || supportTargets.includes(room) || supportLowImpact.includes(room) || portableRelated;
    };

    const normalizeSingleCapacityRoom = (room: string) => {
      const cap = this.dynamicCapacity[room] ?? (['CT', 'MRI', '治療'].includes(room) ? 3 : 1);
      if (cap !== 1) return;
      if (roomNeedsFlexibleCoverage(room)) return;
      ...
      const halfDayOnly = members.some(m => !entryCoversShift(m, 'AM') || !entryCoversShift(m, 'PM'));
      if (halfDayOnly) return;
...
export default function App(): any {
  ...
  const getDayWarnings = (dayId: string): WarningInfo[] => {
    const w: WarningInfo[] = [];
    const cells = allDays[dayId] || {};
    const dayObj = days.find(d => d.id === dayId) || null;
    const dayChar = dayObj?.label.match(/\((.*?)\)/)?.[1] || "";
    const staffMap: Record<string, string[]> = {};

    const getBaseCapacity = (room: string) => {
      if (room === "受付") return customRules.capacity?.受付 ?? 2;
      return customRules.capacity?.[room] ?? (["CT", "MRI", "治療"].includes(room) ? 3 : 1);
    };
    const getClosedFlags = (room: string) => {
      let amClosed = false;
      let pmClosed = false;
      let allClosed = false;
      (customRules.closedRooms || []).forEach((rule: any) => {
        if (rule.room !== room || rule.day !== dayChar) return;
        if (rule.time === "全日") allClosed = true;
        else if (rule.time === "(AM)") amClosed = true;
        else if (rule.time === "(PM)") pmClosed = true;
      });
      if (amClosed && pmClosed) allClosed = true;
      return { amClosed, pmClosed, allClosed };
    };
    const overlaps = (a: { start: number; end: number }, b: { start: number; end: number }) => a.start < b.end && b.start < a.end;
    const formatSeg = (seg: { start: number; end: number }) => {
      if (seg.start <= 0 && seg.end >= 24 * 60) return "終日";
      if (seg.start <= 0 && seg.end === 12 * 60) return "AM";
      if (seg.start === 12 * 60 && seg.end >= 24 * 60) return "PM";
      return `${formatMinutesToClock(seg.start)}〜${formatMinutesToClock(seg.end)}`;
    };
    ...
    softNgPairs.forEach((ng: any) => {
      const s1 = extractStaffName(ng.s1);
      const s2 = extractStaffName(ng.s2);
      ROOM_SECTIONS.forEach(room => {
    ...
    Object.entries(staffMap).forEach(([staff, rms]) => {
      ...
      const dayCount = rms.filter(r => {
        const m = split(cells[r]).find(x => extractStaffName(x) === staff);
        return m && !m.includes("17:") && !m.includes("18:") && !m.includes("19:") && !m.includes("22:");
      }).length;
      if (dayCount >= limit) {
        w.push({ level: 'orange', title: '兼務限界', staff, msg: `${staff}さんが日中 ${dayCount}部屋を担当中` });
      }
    });

    const targetRooms = Array.from(new Set(split(customRules.alertEmptyRooms || "CT,MRI,治療,RI,1号室,2号室,3号室,5号室,透視（6号）,透視（11号）,MMG,骨塩,パノラマCT,ポータブル,DSA,検像").filter(room => room && room !== "受付ヘルプ")));
    targetRooms.forEach(room => {
      const members = split(cells[room] || "");
      const baseCap = getBaseCapacity(room);
      const closed = getClosedFlags(room);
      if (closed.allClosed) return;

      const actual = members.filter(m => !ROLE_PLACEHOLDERS.includes(extractStaffName(m)));
      const placeholders = members.filter(m => ROLE_PLACEHOLDERS.includes(extractStaffName(m)));
      const boundaries = Array.from(new Set([
        ...
        ...members.flatMap(member => {
          ...
          const tag = member.substring(core.length);
          const range = parseTimeTagRange(tag || "") || { start: 0, end: 24 * 60 };
          return [range.start, range.end];
        })
      ])).sort((a, b) => a - b);

      let roomHasHardShortage = false;
      for (let i = 0; i < boundaries.length - 1; i++) {
        const seg = { start: boundaries[i], end: boundaries[i + 1] };
        if (seg.end <= seg.start) continue;

        let required = baseCap;
        if (seg.end <= 12 * 60 && closed.amClosed) required = 0;
        else if (seg.start >= 12 * 60 && closed.pmClosed) required = 0;
        if (required <= 0) continue;

        const extraRequired = placeholders.filter(member => {
          const core = extractStaffName(member);
          const tag = member.substring(core.length);
          const range = parseTimeTagRange(tag || "") || { start: 0, end: 24 * 60 };
          return overlaps(range, seg);
        }).length;
        required += extraRequired;

        const covered = actual.filter(member => {
          const core = extractStaffName(member);
          const tag = member.substring(core.length);
          const range = parseTimeTagRange(tag || "") || { start: 0, end: 24 * 60 };
          return overlaps(range, seg);
        ...
        if (covered < required) {
          roomHasHardShortage = true;
          const label = formatSeg(seg);
          w.push({
            level: 'red',
            title: '割当不能',
            room,
            msg: `「${room}」の${label}は必要${required}人に対して${covered}人です。絶対条件を優先したため、これ以上は安全に割り当てできていません。`
          });
        }
      }

      if (!roomHasHardShortage && actual.length === 0) {
        w.push({ level: 'yellow', title: '空室', room, msg: `「${room}」の担当者がいません` });
      }
    });

    const uTarget = customRules.capacity?.受付 ?? 2;
    if (split(cells["受付"]).reduce((sum: number, m: string) => sum + getStaffAmount(m), 0) < uTarget && split(cells["受付ヘルプ"]).length === 0) {
    ...
    if (curIdx > 0 && !days[curIdx - 1].isPublicHoliday) {
      split(customRules.noConsecutiveRooms).forEach(room => {
        ...
        split(cells[room]).map(extractStaffName).filter(n => prev.includes(n)).forEach(n => {
          w.push({ level: 'red', title: '連日注意', staff: n, room, msg: `${n}さんが「${room}」に連日入っています` });
        });
      });
    }
    ...
    allStaff.filter(s => !absent.has(s) && !assigned.has(s)).forEach(staff => {
      w.push({ level: 'red', title: '未配置', staff, msg: `${staff}さんが1日を通してどの業務にも配置されていません` });
    });

    const pSMap: Record<string, number> = {};
    for (let i = 0; i <= curIdx; i++) {
      split((allDays[days[i].id] || {})["ポータブル"]).map(extractStaffName).forEach(m => {
        pSMap[m] = (pSMap[m] || 0) + 1;
      });
    }
    Object.entries(pSMap).forEach(([staff, count]) => {
      if (count >= 2 && split((allDays[dayId] || {})["ポータブル"]).map(extractStaffName).includes(staff)) {
        w.push({ level: 'orange', title: '頻出注意', staff, msg: `${staff}さんが今週${count}回目のポータブルです` });
      }
    });
