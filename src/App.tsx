...
const globalStyle = `
  ...
  * { box-sizing: border-box; }
  ...
  @media print {
    ...
    /* 🌟 印刷時に名前が大量に入っても崩れないようword-breakとmax-widthを指定 */
...
const KEY_ALL_DAYS = "shifto_alldays_v102"; // マイグレーション用キー更新
...
for (let h = 8; h <= 19; h++) {
  for (let m = 0; m < 60; m += 15) {
    // 🌟 修正: 8:00は始業時間であり、シフトの途中入場（遅番など）の区切りとしては不要なためスキップ
    if (h === 8 && m === 0) continue; 
    const mm = pad(m);
...
// ============== 🌟 純粋関数への切り出し ==============
// autoAssign が外部のコンポーネント状態に依存しないよう、必要なデータを Context として渡します。
...
// 重複していた if/else if チェーンをヘルパー関数に抽出
...
const executeAutoAssign = (day: any, prevDay: any, pastDays: any[], ctx: AutoAssignContext) => {
  ...
  const processPostTasks = () => {
    ...
    if (!skipSections.includes("昼当番")) {
      ...
      // 🌟 追加: RI担当者は無条件で昼当番に最優先アサイン（NG設定されていなければ）
...
export default function App() {
  ...
  // 🌟 修正: Reactのアンチパターン（setState内でsetState）を解消
  const handleUndo = () => {
