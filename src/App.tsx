...
export class AutoAssigner {
  ...
  processPostTasks() {
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((r: any) => {
      ...
      while ((cA < tC || cP < tC) && uG1.length > 0) { const ci = uG1.findIndex(s => !this.isForbidden(s, r.target) && !this.isHalfDayBlocked(s, r.target).hard && !this.hasNGPair(s, cM.map(extractStaffName), false) && !(r.target === "MMG" && !this.isMmgCapable(s)) && this.canAddKenmu(s, r.target) && !(cP >= tC && this.blockMap.get(s) === 'AM') && !(cA >= tC && this.blockMap.get(s) === 'PM') && !this.isTimeTagBlockedByFullDayRule(r.target, this.blockMap.get(s) || "")); if (ci === -1) break; const st = uG1[ci]; uG1.splice(ci, 1); let t = this.getPreferredWorkTag(st); if (!t) { if (cA >= tC) t = "(PM)"; else if (cP >= tC) t = "(AM)"; } this.blockMap.set(st, t === "" ? 'ALL' : t === "(AM)" ? 'PM' : 'AM'); cM.push(`${st}${t}`); if (t === "(AM)") cA++; else if (t === "(PM)") cP++; else { cA++; cP++; } this.addUsage(st, t ? 0.5 : 1); this.log(`🙌 [負担軽減] 余剰の ${st} を ${r.target} に専任配置`); } this.dayCells[r.target] = join(cM);
    });
...
export default function App(): any {
  ...
  const getDayWarnings = (dayId: string): WarningInfo[] => { 
    ...
    if (curIdx > 0) { split(customRules.noConsecutiveRooms).forEach(room => { const prev = curIdx > 0 && !days[curIdx-1].isPublicHoliday ? split(allDays[days[curIdx-1].id]?.[room]).map(extractStaffName) : []; split(cells[room]).map(extractStaffName).forEach(n => { const weekCount = days.slice(0, curIdx).filter(d => split(allDays[d.id]?.[room]).map(extractStaffName).includes(n)).length; if (prev.includes(n)) w.push({ level: 'red', title: '連日注意', staff: n, room, msg: `${n}さんが「${room}」に連日入っています` }); else if (weekCount > 0) w.push({ level: 'orange', title: '週内重複', staff: n, room, msg: `${n}さんが「${room}」に今週${weekCount + 1}回目で入っています` }); }); }); } 
