...
export class AutoAssigner {
  ...
  private consumeAvailability(staff: string, tag: string) {
    const exactTag = this.timeTagMap.get(staff);
    const current = this.blockMap.get(staff);
    if (!tag) {
      this.blockMap.set(staff, 'ALL');
      return;
    }
    if ((exactTag && tag === exactTag) || current === 'AM' || current === 'PM') {
      this.blockMap.set(staff, 'ALL');
      return;
    }
    if (tag === "(AM)") {
  ...
  execute(): DayData {
      ...
      priority.forEach((room: string) => {
         ...
         while (getAmt(current) < eff.cap) {
            const freeStaff = sortedAvail.find((s: string) => { 
                ...
                const b = this.blockMap.get(s); const preferredTag = this.getPreferredWorkTag(s); if ((eff.pmClosed && b === 'AM') || (eff.amClosed && b === 'PM') || this.isTimeTagBlockedByFullDayRule(room, preferredTag)) return false; return true; 
            });
            if (!freeStaff) break;
    ...
    PRIORITY_LIST.forEach((room: string) => {
      ...
      if (room === "受付") {
        ...
        for (const n of ukeMo) { if (this.initialAvailAll.includes(n) && !this.isUsed(n) && !cUke.map(extractStaffName).includes(n)) { const b = this.blockMap.get(n); if (b === 'ALL') continue; const t = this.getPreferredWorkTag(n); cUke.push(`${n}${t}`); this.addUsage(n, t?0.5:1); this.consumeAvailability(n, t); } }
        const cUkeAmt = cUke.reduce((sum: number, m: string) => sum + getStaffAmount(m), 0); let needUke = targetCount - cUkeAmt;
        if (needUke > 0 && !linkedTargetRooms.includes(room)) { const pUke = this.pick(this.initialAvailReception, this.initialAvailReception, Math.ceil(needUke), "受付", cUke); pUke.forEach((n: string) => { const t = this.getPreferredWorkTag(n); cUke.push(`${n}${t}`); this.addUsage(n, t?0.5:1); this.consumeAvailability(n, t); }); }
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number, forcedNeedTag: string = "") {
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const c = pC[0], b = this.blockMap.get(c); let t = "", f = 1; if (b === 'AM' || b === 'PM') { t = this.getPreferredWorkTag(c); f = 0.5; this.consumeAvailability(c, t); } else { if (needTag) { t = needTag; f = 0.5; this.consumeAvailability(c, needTag); } else if (eff.pmClosed) { t = "(AM)"; f = 0.5; this.consumeAvailability(c, "(AM)"); } else if (eff.amClosed) { t = "(PM)"; f = 0.5; this.consumeAvailability(c, "(PM)"); } else { t = ""; f = 1; this.consumeAvailability(c, ""); } }
  ...
  processPostTasks() {
    ...
    this.initialAvailSupport.forEach(staff => { if (this.isUsed(staff)) return; let asg = false; for (const rm of split(this.ctx.customRules.supportTargetRooms)) { if (this.skipSections.includes(rm) || this.isForbidden(staff, rm) || rm === "透析後胸部") continue; let c = split(this.dayCells[rm]); if (c.length > 0 && !c.map(extractStaffName).includes(staff) && !this.hasNGPair(staff, c.map(extractStaffName), false) && !this.isHardNoConsecutive(staff, rm)) { const t = this.getPreferredWorkTag(staff); if (this.isTimeTagBlockedByFullDayRule(rm, t)) continue; this.dayCells[rm] = join([...c, `${staff}${t}`]); this.addUsage(staff, t ? 0.5 : 1); this.consumeAvailability(staff, t); asg = true; break; } } if (!asg) { for (const rm of split(this.ctx.customRules.supportTargetRooms)) { if (this.skipSections.includes(rm) || this.isForbidden(staff, rm) || rm === "透析後胸部") continue; if (!split(this.dayCells[rm]).length && !this.isHardNoConsecutive(staff, rm) && this.canAddKenmu(staff, rm)) { const t = this.getPreferredWorkTag(staff); if (this.isTimeTagBlockedByFullDayRule(rm, t)) continue; this.dayCells[rm] = `${staff}${t}`; this.addUsage(staff, t ? 0.5 : 1); this.consumeAvailability(staff, t); break; } } } });
    ...
    (this.ctx.customRules.swapRules || []).forEach((r: any) => {
      ...
      if (!tM.some(m => { const c = extractStaffName(m); return !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, r.targetRoom) && !this.isHardNoConsecutive(c, r.targetRoom) && !this.isHalfDayBlocked(c, r.targetRoom).hard && this.canAddKenmu(c, r.targetRoom, true); })) {
        ...
        for (const sSR of split(r.sourceRooms).sort((a, b) => this.getRescueSourceScore(parseRoomCond(a).r, r.targetRoom) - this.getRescueSourceScore(parseRoomCond(b).r, r.targetRoom))) {
          ...
          for (const sm of sC) { const sCo = extractStaffName(sm); const tTK = tM.find(m => !this.isForbidden(extractStaffName(m), sR) && !this.isHalfDayBlocked(extractStaffName(m), sR).hard && !this.hasNGPair(extractStaffName(m), sM.map(extractStaffName), false) && this.canAddKenmu(extractStaffName(m), sR, true)); if (tTK && this.canAddKenmu(sCo, r.targetRoom, true)) { const swappedTrigger = tM.map(m => m === tTK ? m.replace(extractStaffName(tTK), sCo) : m); const swappedSource = sM.map(m => m === sm ? m.replace(sCo, extractStaffName(tTK)) : m); this.dayCells[r.triggerRoom] = join(swappedTrigger); this.dayCells[sR] = join(swappedSource); const moved = swappedTrigger.find(m => extractStaffName(m) === sCo) || `${sCo}${this.getPreferredWorkTag(sCo)}`; this.dayCells[r.triggerRoom] = join(swappedTrigger.filter(m => extractStaffName(m) !== sCo)); this.dayCells[r.targetRoom] = join([...split(this.dayCells[r.targetRoom]), moved]); this.addUsage(sCo, getStaffAmount(moved)); this.updateBlockMapAfterKenmu(sCo, moved); this.log(`🔄 [交換ルール] ${r.triggerRoom}の${extractStaffName(tTK)}と ${sR}の${sCo}を交換し、${sCo} を ${r.targetRoom} に配置`); sw = true; break; } } if (sw) break;
        }
      }
    });
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((r: any) => {
      ...
      while ((cA < tC || cP < tC) && uG1.length > 0) { const ci = uG1.findIndex(s => !this.isForbidden(s, r.target) && !this.isHalfDayBlocked(s, r.target).hard && !this.hasNGPair(s, cM.map(extractStaffName), false) && !this.isHardNoConsecutive(s, r.target) && !(r.target === "MMG" && !this.isMmgCapable(s)) && this.canAddKenmu(s, r.target) && !(cP >= tC && this.blockMap.get(s) === 'AM') && !(cA >= tC && this.blockMap.get(s) === 'PM') && !this.isTimeTagBlockedByFullDayRule(r.target, this.getPreferredWorkTag(s))); if (ci === -1) break; const st = uG1[ci]; uG1.splice(ci, 1); let t = this.getPreferredWorkTag(st); if (!t) { if (cA >= tC) t = "(PM)"; else if (cP >= tC) t = "(AM)"; } this.consumeAvailability(st, t); cM.push(`${st}${t}`); if (t === "(AM)") cA++; else if (t === "(PM)") cP++; else { cA++; cP++; } this.addUsage(st, t ? 0.5 : 1); this.log(`🙌 [負担軽減] 余剰の ${st} を ${r.target} に専任配置`); } this.dayCells[r.target] = join(cM);
    });
    ...
    if (!this.skipSections.includes("昼当番")) {
      ...
      const femaleStaff = split((this.ctx.customRules as any).femaleStaffList || "").map(extractStaffName);
      const addFemaleLunch = () => {
        if (femaleStaff.length > 0 && cL.some(n => femaleStaff.includes(extractStaffName(n)))) return;
        const fallbackFemales = femaleStaff.length > 0 ? femaleStaff : split(this.dayCells["MMG"]).map(extractStaffName);
        const priorityRooms = Array.from(new Set(["RI", ...split(this.ctx.customRules.lunchPrioritySections ?? "RI,1号室,2号室,3号室,5号室,CT"), "MMG", "1号室", "2号室", "3号室", "5号室"]));
        for (const room of priorityRooms) {
          const picked = split(this.dayCells[room] || "").map(extractStaffName).find(n => fallbackFemales.includes(n) && !cL.includes(n) && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, cL, false));
          if (picked) { cL.push(picked); return; }
        }
        const freeFemale = this.initialAvailGeneral.find((n: string) => fallbackFemales.includes(n) && !cL.includes(n) && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, cL, false));
        if (freeFemale) cL.push(freeFemale);
      };
      addFemaleLunch();
...
export default function App(): any {
  ...
  const handleClearWorkDay = () => { if (window.confirm(`${cur.label} の業務欄をクリアしますか？`)) { const workSections = [...RENDER_GROUPS[1].sections, ...RENDER_GROUPS[2].sections]; setAllDaysWithHistory((prev: any) => { const nextCells = { ...(prev[cur.id] || cur.cells) }; workSections.forEach(sec => { nextCells[sec] = ""; }); return { ...prev, [cur.id]: nextCells }; }); } };
  const handleClearWorkWeek = () => { if (window.confirm(`表示中の業務欄を1週間分すべてクリアしますか？`)) { const workSections = [...RENDER_GROUPS[1].sections, ...RENDER_GROUPS[2].sections]; setAllDaysWithHistory((prev: any) => { const nextState = { ...prev }; days.forEach(d => { const nextCells = { ...(prev[d.id] || d.cells) }; workSections.forEach(sec => { nextCells[sec] = ""; }); nextState[d.id] = nextCells; }); return nextState; }); } };
      ...
      <div className="no-print" style={{ display: activeTab === 'rules' ? 'block' : 'none' }}>
        <div style={{ ...panelStyle(), marginBottom: 32 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
             ...
             <div><label style={{ fontSize: 19, fontWeight: 800, marginBottom: 10, display: "block" }}>女性スタッフ</label><textarea className="name-textarea" placeholder="例: 佐藤(さとう)" value={customRules.femaleStaffList || ""} onChange={e => setCustomRules({...customRules, femaleStaffList: e.target.value})} /></div>
          </div>
        </div>
