...
const cellStyle = (isHeader=false, isHoliday=false, isSelected=false, isLabel=false, isZebra=false): React.CSSProperties => ({
  padding: isHeader ? "10px 12px" : "6px 8px",
  background: isHoliday ? "#fef2f2" : isSelected ? "#eff6ff" : isLabel ? "#f8fafc" : isZebra ? "#f8fafc" : "#ffffff",
  color: isHoliday ? "#ef4444" : isLabel ? "#334155" : "#1e293b",
  fontWeight: isHeader||isLabel ? 700 : 500,
  fontSize: 14,
  verticalAlign: "top",
...
export default function App(): any {
  ...
  const getDailyStats = (dayId: string) => { 
    ...
    const workingStaff = allStaff.filter(s => !absentMems.some(m => ex(m) === s && !m.includes("(AM)") && !m.includes("(PM)"))); 
