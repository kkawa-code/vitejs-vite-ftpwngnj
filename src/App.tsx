...
const globalStyle = `
  ...
  * { box-sizing: border-box; }
  ...
  .sticky-header { position: sticky; top: 0; z-index: 30; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(4px); padding: 12px; margin-bottom: 20px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 12px; border: 1px solid #e2e8f0; }
...
const KEY_ALL_DAYS = "shifto_alldays_v106"; 
const KEY_MONTHLY = "shifto_monthly_v106"; 
const KEY_RULES = "shifto_rules_v106";
...
const WeekCalendarPicker = ({ targetMonday, onChange, nationalHolidays, customHolidays }: any) => {
  ...
  return (
    <div style={{ position: "relative" }}>
      <button className="btn-hover" onClick={() => setIsOpen(!isOpen)} style={{ ...btnStyle("#fff", "#2563eb"), border: "1px solid #bfdbfe" }}>
        📅 {targetMonday} の週 ▼
      </button>
      {isOpen && (
        <>
      ...
      if (!current.some(m => m.includes(rule.lateTime))) {
        ...
        const getCandidate = (candidatesList: string[]) => {
          let cand = candidatesList.filter(name => {
            ...
            // 🌟 バグ修正: 午後不在の人（PMがブロックされている人）だけを遅番から除外
    ...
    if (!skipSections.includes("昼当番")) {
      ...
      if (needsUketsukeHelp && !skipSections.includes("受付ヘルプ")) {
        ...
        const getHelp = (exclude: string[]) => {
          ...
          cand = availAll.filter(n => !exclude.includes(n) && !helpMems.map(getCoreName).includes(n) && blockMap.get(n) !== 'ALL' && !isForbidden(n, "受付ヘルプ"));
          if (cand.length > 0) { cand.sort((a, b) => (assignCounts[a] || 0) - (assignCounts[b] || 0)); return cand[0]; }
          return null;
        };
        ...
        if (!picked16) {
          ...
          if (cand.length > 0) { cand.sort((a, b) => (assignCounts[a] || 0) - (assignCounts[b] || 0)); picked16 = cand[0]; }
            ...
            cand = availAll.filter(n => blockMap.get(n) !== 'AM' && !helpMems.map(getCoreName).includes(n) && !excl.includes(n) && !isForbidden(n, "受付ヘルプ"));
            if (cand.length > 0) { cand.sort((a, b) => (assignCounts[a] || 0) - (assignCounts[b] || 0)); picked16 = cand[0]; }
          }
        }
...
export default function App() {
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 12px", width: "100%", boxSizing: "border-box", overflowX: "hidden" }}>
          ...
          <div style={{ paddingTop: 20, borderTop: "2px dashed #e2e8f0", marginTop: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, marginBottom: 20 }}>
              ...
              <div style={{ background: "#e0f2fe", padding: 16, borderRadius: 12, border: "1px solid #bae6fd", gridColumn: "1 / -1" }}>
                ...
                <p style={{ fontSize: 12, color: "#0284c7", marginBottom: 12, fontWeight: 600 }}>「A পুনরায়Bさんが同じ部屋になりそうな時、Bさんを別の部屋に押し出す」ルールです。</p>
