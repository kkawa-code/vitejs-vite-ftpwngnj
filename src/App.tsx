...
export class AutoAssigner {
  ...
  processPostTasks() {
    const sSL = split(this.ctx.customRules.supportStaffList || "").map(extractStaffName); 
    ...
    (this.ctx.customRules.swapRules || []).forEach((rule: any) => {
      ...
      const triggerCanTarget = triggerMembers.some(m => { const c = extractStaffName(m); if (ROLE_PLACEHOLDERS.includes(c) || this.isForbidden(c, targetRoom) || this.isHardNoConsecutive(c, targetRoom) || this.isHB(c, targetRoom).h) return false; return this.canAddKenmu(c, targetRoom, true); });
      if (!triggerCanTarget) {
          ...
          for (const srcStrRoom of swapSources) {
              ...
              let srcCands = srcMembers.filter(m => !ROLE_PLACEHOLDERS.includes(extractStaffName(m)) && !this.isForbidden(extractStaffName(m), targetRoom) && !this.isHB(extractStaffName(m), targetRoom).h && !this.isHardNoConsecutive(extractStaffName(m), targetRoom) && this.canAddKenmu(extractStaffName(m), targetRoom, true) && !this.isForbidden(extractStaffName(m), triggerRoom) && !(this.isFDO(targetRoom) && (m.includes("(AM)") || m.includes("(PM)"))));
              ...
              for (const srcM of srcCands) { const srcCore = extractStaffName(srcM); const targetToKick = triggerMembers.find(m => { const c = extractStaffName(m); return !this.isForbidden(c, srcRoom) && !this.isHB(c, srcRoom).h && !this.hasNGPair(c, srcMembers.map(extractStaffName), false) && this.canAddKenmu(c, srcRoom, true); }); if (targetToKick && this.canAddKenmu(srcCore, targetRoom, true)) { const kickCore = extractStaffName(targetToKick); this.dayCells[triggerRoom] = join(triggerMembers.map(m => m === targetToKick ? m.replace(kickCore, srcCore) : m)); this.dayCells[srcRoom] = join(srcMembers.map(m => m === srcM ? m.replace(srcCore, kickCore) : m)); this.log(`🔄 [玉突き] ${triggerRoom}の${kickCore}と ${srcRoom}の${srcCore}を交換`); swapped = true; break; } }
              if (swapped) break;
          }
      }
    });
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((rule: any) => {
      ...
      const targetCap = this.dynamicCapacity[targetRoom] !== undefined ? this.dynamicCapacity[targetRoom] : (["CT", "MRI", "治療"].includes(targetRoom) ? 3 : 1);
      const effLR = this.getEffectiveTarget(targetRoom, targetCap); if (effLR.al) return;
      ...
      const isOnlySupport = currentMems.length > 0 && currentMems.every(m => sSL.includes(extractStaffName(m))); if (isOnlySupport) { curAm = 0; curPm = 0; }
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((rule: any) => {
      ...
      for (const srcStr of sourceRooms) {
        ...
        split(this.dayCells[srcRoom]).forEach(m => {
          ...
          if (!ROLE_PLACEHOLDERS.includes(core) && !currentMems.map(extractStaffName).includes(core) && !this.isForbidden(core, targetRoom) && !this.isHB(core, targetRoom).h && !this.hasNGPair(core, currentMems.map(extractStaffName), false) && !this.isHardNoConsecutive(core, targetRoom) && (targetRoom === "MMG" ? this.isMmgCapable(core) : true) && this.canAddKenmu(core, targetRoom, true) && !m.includes("17:") && !m.includes("19:") && !(this.isFDO(targetRoom) && (m.includes("(AM)") || m.includes("(PM)")))) {
               let pushStr = m;
               if (targetRoom === "パノラマCT" && srcRoom === "透視（6号）") { if (m.includes("(PM)")) return; pushStr = `${core}(AM)`; }
                 ...
                 else if (effLR.pC) { if (m.includes("(PM)")) return; pushStr = `${core}(AM)`; }
                 else if (effLR.aC) { if (m.includes("(AM)")) return; pushStr = `${core}(PM)`; }
               }
    ...
    ROOM_SECTIONS.forEach(targetRoom => {
      ...
      const targetCap = this.dynamicCapacity[targetRoom] !== undefined ? this.dynamicCapacity[targetRoom] : (["CT", "MRI", "治療"].includes(targetRoom) ? 3 : 1);
      const effResc = this.getEffectiveTarget(targetRoom, targetCap); if (effResc.al) return;
      let currentMems = split(this.dayCells[targetRoom]); let curAm = effResc.aC ? 999 : 0; let curPm = effResc.pC ? 999 : 0; currentMems.forEach(x => { if (x.includes("(AM)")) curAm += 1; else if (x.includes("(PM)")) curPm += 1; else { curAm += 1; curPm += 1; } }); const isOnlySupport = currentMems.length > 0 && currentMems.every(m => sSL.includes(extractStaffName(m))); if (isOnlySupport) { curAm = effResc.aC ? 999 : 0; curPm = effResc.pC ? 999 : 0; } if (curAm >= targetCap && curPm >= targetCap) return;
    ...
    const pM = split(this.dayCells["ポータブル"]); 
    const r2M = split(this.dayCells["2号室"]); 
    const r2C = r2M.map(extractStaffName);
    const r2A = r2M.reduce((s, m) => s + (ROLE_PLACEHOLDERS.includes(extractStaffName(m)) ? 0 : getStaffAmount(m)), 0);

    let nAH = false; let nPH = false;
    r2M.forEach(r2m => {
      const c = extractStaffName(r2m);
      const pm = pM.find(pm => extractStaffName(pm) === c);
      if (pm) { if (pm.includes("(AM)")) nAH = true; else if (pm.includes("(PM)")) nPH = true; else { nAH = true; nPH = true; } }
    });

    if (r2A < 2 && (nAH || nPH)) {
      const fC = (iA: boolean, ex: string[]) => [...sSL, ...this.initialAvailGeneral].filter((s,i,a)=>a.indexOf(s)===i).find(s => {
        if (r2C.includes(s) || this.isForbidden(s,"2号室") || this.isHB(s,"2号室").h || this.hasNGPair(s,r2C,false) || !this.canAddKenmu(s,"2号室") || this.isHardNoConsecutive(s,"2号室")) return false;
        const b = this.blockMap.get(s); if (b === 'ALL') return false; if (iA && b === 'AM') return false; if (!iA && b === 'PM') return false;
    ...
    const dKT = ROOM_SECTIONS.filter(r => !["CT", "MRI", "治療", "RI", "待機", "昼当番", "受付", "受付ヘルプ", "透析後胸部"].includes(r)); 
    dKT.sort((a, b) => { let iA = pL.indexOf(a); if (iA === -1) iA = 999; let iB = pL.indexOf(b); if (iB === -1) iB = 999; return iB - iA; });
    ...
    uG2.forEach(st => {
      ...
      for (const rm of dKT) {
        ...
        const rI = cM.findIndex(m => { 
          ...
          if (ROLE_PLACEHOLDERS.includes(c) || this.hasNGPair(st, cM.filter(x=>x!==m).map(extractStaffName), false)) return false; 
          const cc = this.getStaffTimeCounts(c);
          if (t === "(AM)" && cc.am <= 1) return false;
          if (t === "(PM)" && cc.pm <= 1) return false;
          if (t === "" && cc.am <= 1 && cc.pm <= 1) return false;
          let ca = m.includes("(AM)"); let cp = m.includes("(PM)"); if(!ca && !cp){ca=true;cp=true;} let na = t==="(AM)"; let np = t==="(PM)"; if(!na && !np){na=true;np=true;} if (ca && !na) return false; if (cp && !np) return false; return true; 
        });
        if (rI !== -1) {
          const oS = cM[rI]; const oC = extractStaffName(oS); 
          let oR = "";
          if (t === "(AM)" && !oS.includes("(")) oR = `${oC}(PM)`;
          else if (t === "(PM)" && !oS.includes("(")) oR = `${oC}(AM)`;
          cM[rI] = `${st}${t}`;
          if (oR) cM.push(oR);
          this.dayCells[rm] = join(cM); this.addU(st, t?0.5:1); this.blockMap.set(st, 'ALL'); this.assignCounts[oC] = Math.max(0, (this.assignCounts[oC] || 1) - getStaffAmount(oS)); let am = false; let pm = false; ROOM_SECTIONS.forEach(r => { if (r === "待機" || r === "昼当番" || r === "受付" || r === "受付ヘルプ") return; split(this.dayCells[r]).forEach(m => { if (extractStaffName(m) === oC) { if (m.includes("(AM)")) am = true; else if (m.includes("(PM)")) pm = true; else { am = true; pm = true; } } }); }); if (am && pm) this.blockMap.set(oC, 'ALL'); else if (am) this.blockMap.set(oC, 'AM'); else if (pm) this.blockMap.set(oC, 'PM'); else this.blockMap.set(oC, 'NONE'); this.log(`🪄 [兼務解消] ${st} を専任化し ${oC} の負担軽減`); asg = true; break;
        }
      }
      if (!asg) { 
        for (const rm of rP) { 
           if (this.skipSections.includes(rm) || this.isForbidden(st, rm) || (rm === "MMG" && !this.isMmgCapable(st))) continue; 
           if (["待機", "昼当番", "受付", "受付ヘルプ", "CT", "MRI", "治療", "RI", "透析後胸部"].includes(rm)) continue; 
           if (t !== "" && this.isHB(st, rm).h) continue; 
           const aC = this.cap[rm] ?? (["CT", "MRI", "治療"].includes(rm) ? 3 : 1);
           const e = this.gET(rm, aC); 
           if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC) || !this.canAddKenmu(st, rm) || this.isHardNoConsecutive(st, rm) || (this.isFDO(rm) && (t === "(AM)" || t === "(PM)"))) continue; 
           let cM = split(this.dayCells[rm]); 
           if (cM.reduce((s, m) => s + getStaffAmount(m), 0) >= e.cap) continue;
           if (this.hasNGPair(st, cM.map(extractStaffName), false)) continue; 
           this.dayCells[rm] = join([...cM, `${st}${t}`]); this.addU(st, t?0.5:1); this.blockMap.set(st, 'ALL'); this.log(`♻️ [余剰配置] 余力のあった ${st} を ${rm} に追加配置`); asg = true; break; 
        } 
      }
      if (!asg) {
        for (const fbR of ["3号室", "2号室", "1号室", "5号室"]) {
           if (this.skipSections.includes(fbR) || this.isForbidden(st, fbR)) continue;
           const e = this.gET(fbR, 1);
           if (e.al || (t === "(PM)" && e.pC) || (t === "(AM)" && e.aC) || this.isHardNoConsecutive(st, fbR) || (this.isFDO(fbR) && (t === "(AM)" || t === "(PM)"))) continue;
           let cM = split(this.dayCells[fbR]); 
           if (this.hasNGPair(st, cM.map(extractStaffName), false)) continue;
           this.dayCells[fbR] = join([...cM, `${st}${t}`]); this.addU(st, t?0.5:1); this.blockMap.set(st, 'ALL'); this.log(`🚨 [最終救済] 未配置を防ぐため定員超過でも ${st} を ${fbR} に強制配置`); asg = true; break; 
        }
      }
    });

    this.logPhase("仕上げ（最後に配置）");
    if (!this.skipSections.includes("昼当番")) {
      ...
      (this.ctx.customRules.lunchRoleRules || []).forEach((r: any) => { if (r.day === "毎日" || r.day === dC) { const rS = split(this.ctx.monthlyAssign[r.role] || "").map(extractStaffName); const tM = split(this.dayCells[r.role] || "").map(extractStaffName); let sl: string | null = null; for (const s of split(r.sourceRooms)) { const { r: sR, min } = parseRoomCond(s); const rM = split(this.dayCells[sR] || ""); const rq = min > 0 ? min : (this.dynamicCapacity[sR] || 1); if (rM.reduce((sum, m) => sum + getStaffAmount(m), 0) >= rq) { sl = rM.map(extractStaffName).find(n => rS.includes(n) && !tM.includes(n) && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, cL, false)) || null; } if (sl) break; } if (!sl) sl = tM.find(n => !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, cL, false)) || null; if (sl && !cL.includes(sl) && cL.length < lT) { cL.push(sl); } } });
      const riM = split(this.dayCells["RI"]).map(extractStaffName); riM.forEach(n => { if (!cL.includes(n) && cL.length < lT && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, cL, false)) cL.push(n); });
      split(this.ctx.customRules.lunchPrioritySections ?? "RI,1号室,2号室,3号室,5号室,CT").forEach(sc => { split(this.dayCells[sc]).forEach(n => { const core = extractStaffName(n); if (!cL.includes(core) && cL.length < lT && !this.isForbidden(core, "昼当番") && !this.hasNGPair(core, cL, false)) cL.push(core); }); });
      if (cL.length < lT) { (this.ctx.customRules.lunchConditional || []).forEach((cond: any) => { const sM = split(this.dayCells[cond.section]); if (sM.length >= Number(cond.min)) { let p = 0; for (const n of sM) { if (p >= Number(cond.out) || cL.length >= lT) break; const core = extractStaffName(n); if (!cL.includes(core) && !this.isForbidden(core, "昼当番") && !this.hasNGPair(core, cL, false)) { cL.push(core); p++; } } } }); }
