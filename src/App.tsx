...
export class AutoAssigner {
  ...
  private getAbsenceHelpTagsForRoom(room: string): string[] {
    ...
    return absentEntries.flatMap((entry: string) => {
      ...
      return roomMembers.includes(staff) ? [helpTag] : [];
    });
  }
    ...
    PRIORITY_LIST.forEach((room: string) => {
      ...
      if (room === "受付") {
        ...
        if (!((this.ctx.customRules.linkedRooms || []).some((r: any) => r.target === room)) && !hasPF) {
          this.fill(cand, room, pList, targetCount, phTags);
          absenceHelpTags.forEach((tag: string) => {
            const currentAmt = split(this.dayCells[room] || "").reduce((sum: number, m: string) => sum + getStaffAmount(m), 0);
            this.fill(cand, room, pList, currentAmt + getTagTimeWeight(tag), tag);
          });
        }
      }
    });
