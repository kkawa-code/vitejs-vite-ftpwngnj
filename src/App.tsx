...
export const KEY_ALL_DAYS = "shifto_alldays_v288"; export const KEY_MONTHLY = "shifto_monthly_v288"; export const KEY_RULES = "shifto_rules_v288";
...
export const RCard = ({ bg, b, c, ic, t, d, children }: any) => ( <div style={{ background: bg, padding: 24, borderRadius: 12, border: `2px solid ${b}`, marginBottom: 20 }}><h5 style={{ margin: "0 0 12px 0", color: c, fontSize: 18, fontWeight: 800 }}>{ic} {t}</h5>{d && <p style={{ fontSize: 15, color: "#166534", marginTop: 0, marginBottom: 16 }}>{d}</p>}{children}</div> );
...
export const StaffSel = ({v, onChange, list, ph="スタッフ", w}:any) => <select value={v} onChange={e=>onChange(e.target.value)} className="rule-sel" style={{width:w}}><option value="">{ph}</option>{list.map((s:any)=><option key={s} value={s}>{s}</option>)}</select>;
export const RoomSel = ({v, onChange, list, ph="場所", w}:any) => <select value={v} onChange={e=>onChange(e.target.value)} className="rule-sel" style={{width:w}}><option value="">{ph}</option>{list.map((s:any)=><option key={s} value={s}>{s}</option>)}</select>;
...
export const renderLog = (logStr: string, i: number) => {
  ...
  else if (category.includes("救済") || category.includes("代打") || category.includes("最終救済")) { bg = "#fff7ed"; border = "#fed7aa"; color = "#9a3412"; badgeBg = "#ffedd5"; badgeColor = "#c2410c"; }
  ...
  else if (category.includes("余剰")) { bg = "#fdf4ff"; border = "#f5d0fe"; color = "#701a75"; badgeBg = "#fae8ff"; badgeColor = "#86198f"; }
    ...
    uGen2.forEach(staff => {
      ...
      if (!assigned) {
        for (const fallbackRoom of ["3号室", "2号室", "1号室", "5号室"]) {
           ...
           if (this.hasNGPair(staff, currentMems.map(extractStaffName), false)) continue;
           this.dayCells[fallbackRoom] = join([...currentMems, `${staff}${tag}`]); this.addU(staff, tag?0.5:1); this.blockMap.set(staff, 'ALL'); this.log(`🚨 [最終救済] 未配置を防ぐため定員超過でも ${staff} を ${fallbackRoom} に強制配置`); assigned = true; break; 
        }
      }
    });
...
export default function App(): any {
  ...
  const getDailyStats = (dayId: string) => { 
    ...
    const workingStaff = allStaff.filter(s => { const isFullAbsent = absentMems.some(m => extractStaffName(m) === s && !m.includes("(AM)") && !m.includes("(PM)")); return !isFullAbsent; }); 
    const staffTime: Record<string, {am: boolean, pm: boolean}> = {};
    workingStaff.forEach(s => staffTime[s] = {am: false, pm: false});
    absentMems.forEach(m => { const c = extractStaffName(m); if (staffTime[c]) { if (m.includes("(AM)")) staffTime[c].am = true; if (m.includes("(PM)")) staffTime[c].pm = true; } });
  ...
  const getDayWarnings = (dayId: string): WarningInfo[] => { const w: WarningInfo[] = []; const cells = allDays[dayId] || {}; const staffMap: Record<string, string[]> = {}; ROOM_SECTIONS.forEach(room => { split(cells[room]).forEach(m => { const core = extractStaffName(m); if(!staffMap[core]) staffMap[core]=[]; if(!staffMap[core].includes(room)) staffMap[core].push(room); }) }); const softNgPairs = (customRules.ngPairs || []).filter((p: any) => p.level === "soft"); softNgPairs.forEach((ng: any) => { const s1 = extractStaffName(ng.s1); const s2 = extractStaffName(ng.s2); ROOM_SECTIONS.forEach(room => { const mems = split(cells[room]).map(extractStaffName); if (mems.includes(s1) && mems.includes(s2)) w.push({ level: 'yellow', title: '回避特例', room, msg: `なるべくNGペア（${s1} と ${s2}）が「${room}」で同室です` }); }); }); Object.entries(staffMap).forEach(([staff, rms]) => { const limit = customRules.alertMaxKenmu || 3; const dayCount = rms.filter(r => { const m = split(cells[r]).find(x => extractStaffName(x) === staff); return m && !m.includes("17:") && !m.includes("18:") && !m.includes("19:") && !m.includes("22:"); }).length; if(dayCount >= limit) w.push({ level: 'orange', title: '兼務限界', staff, msg: `${staff}さんが日中 ${dayCount}部屋を担当中` }); }); const targetEmptyRooms = split(customRules.alertEmptyRooms || "CT,MRI,治療,RI,1号室,2号室,3号室,5号室,透視（6号）,透視（11号）,MMG,骨塩,パノラマCT,ポータブル,DSA,検像"); targetEmptyRooms.forEach(room => { if (split(cells[room]).length === 0) w.push({ level: 'yellow', title: '空室', room, msg: `「${room}」の担当者がいません` }); }); const uTarget = customRules.capacity?.受付 ?? 2; if (split(cells["受付"]).reduce((sum: number, m: string) => sum + getStaffAmount(m), 0) < uTarget && split(cells["受付ヘルプ"]).length === 0) { w.push({ level: 'yellow', title: '受付不足', room: '受付', msg: `受付が${uTarget}名未満ですが、受付ヘルプがいません` }); } const curIdx = days.findIndex(d => d.id === dayId); if (curIdx > 0 && !days[curIdx-1].isPublicHoliday) { split(customRules.noConsecutiveRooms).forEach(room => { const prev = split(allDays[days[curIdx-1].id]?.[room]).map(extractStaffName); split(cells[room]).map(extractStaffName).filter(n => prev.includes(n)).forEach(n => w.push({ level: 'red', title: '連日注意', staff: n, room, msg: `${n}さんが「${room}」に連日入っています` })); }); } 
    const stats = getDailyStats(dayId); stats.unassigned.forEach(item => { if (item.includes("(AM未配置)")) w.push({ level: 'red', title: '半日未配置', staff: extractStaffName(item), msg: `${extractStaffName(item)}さんの午前が未配置です` }); else if (item.includes("(PM未配置)")) w.push({ level: 'red', title: '半日未配置', staff: extractStaffName(item), msg: `${extractStaffName(item)}さんの午後が未配置です` }); else w.push({ level: 'red', title: '未配置', staff: item, msg: `${item}さんが1日を通して配置されていません` }); }); return w; 
  };
  ...
  return (
    <div style={{ maxWidth: "98%", margin: "0 auto", padding: "24px", boxSizing: "border-box" }}>
      ...
      <div className="no-print" style={{ ...panelStyle(), display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, padding: "20px 32px", background: "linear-gradient(to right, #ffffff, #f8fafc)" }}>
        <h2 style={{ margin: 0, color: "#0f172a", fontSize: 26, fontWeight: 900 }}>勤務割付システム Ver 2.90</h2>
                          ...
                          {!day.isPublicHoliday && (
                            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", lineHeight: "1.4" }}>
                              {currentMems.map((m, mIdx) => {
                                ...
                                // 色分けルール整理：上限超過やエラー時のみ赤くする
                                if (roomCount >= 3) { tagBg = "#ffedd5"; tagColor = "#9a3412"; tagBorder = "#fdba74"; }
                ...
                <div key={idx} style={{background:"#fff", padding:"12px 16px", border:"2px dashed #fde047", borderRadius:8, marginBottom:8}}>
                  <div className="rule-row">
                    ...
                    {rule.type === "empty_room_swap" ? ( <><RoomSel v={rule.section} onChange={(v:any)=>updateRule("emergencies", idx, "section", v)} list={ROOM_SECTIONS} ph="監視する部屋" /><span className="rule-label" style={{color:"#854d0e"}}>が空の時</span></> ) : (
                      <>
                        <span className="rule-label" style={{color:"#854d0e"}}>出勤</span><NumInp v={rule.threshold ?? 16} onChange={(v:any)=>updateRule("emergencies", idx, "threshold", v)} w={60} /><span className="rule-label" style={{color:"#854d0e"}}>人以下で発動➔</span>
            ...
            <RCard bg="#fff7ed" b="#fed7aa" c="#c2410c" ic="🔄" t="代打ルール">
              {(customRules.substitutes || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 16, alignItems: "center", background: "#fff", padding: "16px", borderRadius: 8, border: "1px solid #fdba74", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
                    ...
                    <span style={{ fontSize: 17, fontWeight: 800, color: "#c2410c" }}>を</span><RoomSel v={rule.section} onChange={(v:any)=>updateRule("substitutes", idx, "section", v)} list={ROOM_SECTIONS} /><span style={{ fontSize: 17, fontWeight: 800, color: "#c2410c" }}>に優先</span><DelBtn onClick={()=>removeRule("substitutes", idx)} />
                  </div>
              ))}
          ...
          <div style={{ borderLeft: "8px solid #8b5cf6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RCard bg="#eef2ff" b="#c7d2fe" c="#4338ca" ic="🍱" t="昼当番ルール">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24, background: "#fff", padding: "12px 20px", borderRadius: 10, border: "1px solid #c7d2fe", width: "fit-content" }}>
                  <span style={{ fontSize: 18, fontWeight: 800, color: "#3730a3" }}>基本の人数:</span><NumInp v={customRules.lunchBaseCount ?? 3} onChange={(v:any)=>setCustomRules({...customRules, lunchBaseCount: v})} w={80} />
              </div>
