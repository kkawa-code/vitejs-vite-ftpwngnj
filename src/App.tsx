...
export function parseAbsenceHelpMap(dayCells?: Record<string, string>): Record<string, string> {
  if (!dayCells) return {};
  const raw = (dayCells[ABSENCE_HELP_KEY] || "{}").trim();
  if (!raw) return {};
  ...
  try {
    ...
    while ((m = re.exec(normalized)) !== null) map[m[1]] = m[2];
    return map;
  }
}
...
export function parseClockTagToMinutes(clock: string): number {
  const [h, m] = clock.split(":").map(Number);
  return h * 60 + m;
}

export function getTagRange(tag: string): { start: number; end: number } {
  if (!tag) return { start: 0, end: 24 * 60 };
  if (tag.includes("(AM)")) return { start: 0, end: 12 * 60 };
  if (tag.includes("(PM)")) return { start: 12 * 60, end: 24 * 60 };
  let m = tag.match(/\(〜(\d{1,2}:\d{2})\)/);
  if (m) return { start: 0, end: parseClockTagToMinutes(m[1]) };
  m = tag.match(/\((\d{1,2}:\d{2})〜\)/);
  if (m) return { start: parseClockTagToMinutes(m[1]), end: 24 * 60 };
  m = tag.match(/\((\d{1,2}:\d{2})〜(\d{1,2}:\d{2})\)/);
  if (m) return { start: parseClockTagToMinutes(m[1]), end: parseClockTagToMinutes(m[2]) };
  return { start: 0, end: 24 * 60 };
}

export function tagsOverlap(tagA: string, tagB: string): boolean {
  const a = getTagRange(tagA);
  const b = getTagRange(tagB);
  return a.start < b.end && b.start < a.end;
}

export function tagContains(availableTag: string, needTag: string): boolean {
  const a = getTagRange(availableTag);
  const n = getTagRange(needTag);
  return a.start <= n.start && a.end >= n.end;
}

export function coversAMWithTag(tag: string): boolean {
  const r = getTagRange(tag);
  return r.start < 12 * 60 && r.end > 0;
}

export function coversPMWithTag(tag: string): boolean {
  const r = getTagRange(tag);
  return r.end > 12 * 60 && r.start < 24 * 60;
}
...
export class AutoAssigner {
  ...
  getStaffTimeCounts(s: string) { let am = 0; let pm = 0; Object.keys(this.dayCells).forEach(sec => { if (["待機","昼当番","受付","受付ヘルプ"].includes(sec) || REST_SECTIONS.includes(sec)) return; split(this.dayCells[sec]).forEach(m => { if (extractStaffName(m) === s) { const tag = m.substring(extractStaffName(m).length); if (coversAMWithTag(tag)) am++; if (coversPMWithTag(tag)) pm++; } }); }); return { am, pm }; }
  ...
  private consumeAvailabilityAfterAssignment(staff: string, assignedTag: string) {
    ...
    const mUntil = assignedTag.match(/^\(〜(\d{1,2}:\d{2})\)$/);
    if (mUntil) {
      this.blockMap.set(staff, 'AM');
      this.timeTagMap.set(staff, `(${mUntil[1]}〜)`);
      return;
    }
    const mFrom = assignedTag.match(/^\((\d{1,2}:\d{2})〜\)$/);
    if (mFrom) {
      this.blockMap.set(staff, 'PM');
      this.timeTagMap.set(staff, `(〜${mFrom[1]})`);
      return;
    }
    this.blockMap.set(staff, 'ALL');
  }

  private ensureSourceRoomBackfill(staff: string, timeTag: string, targetSection: string) {
    if (!timeTag) return;
    ROOM_SECTIONS.forEach((room: string) => {
      if (room === targetSection) return;
      const current = split(this.dayCells[room] || "");
      const hasStaff = current.some((entry: string) => {
        const core = extractStaffName(entry);
        if (ROLE_PLACEHOLDERS.includes(core) || core !== staff) return false;
        const entryTag = entry.substring(core.length);
        return tagsOverlap(entryTag, timeTag);
      });
      if (!hasStaff) return;
      const placeholder = `${room}枠${timeTag}`;
      if (!current.includes(placeholder)) {
        this.dayCells[room] = join([...current, placeholder]);
        this.log(`🧩 [補充連鎖] ${staff} の ${timeTag} を ${targetSection} へ回すため ${room} に補充枠を追加`);
      }
    });
  }
  ...
  execute(): DayData {
    ...
    PRIORITY_LIST.forEach((room: string) => {
      ...
      if (room === "受付") {
        ...
        for (const n of ukeMo) { if (this.initialAvailAll.includes(n) && !this.isUsed(n) && !cUke.map(extractStaffName).includes(n)) { if (this.blockMap.get(n) === 'ALL') continue; let t = this.getWorkTagForStaff(n); cUke.push(`${n}${t}`); this.addUsage(n, t?0.5:1); this.blockMap.set(n, 'ALL'); } }
        const cUkeAmt = cUke.reduce((sum: number, m: string) => sum + getStaffAmount(m), 0); let needUke = targetCount - cUkeAmt;
        if (needUke > 0 && !linkedTargetRooms.includes(room)) {
          const receptionOnly = this.ctx.activeReceptionStaff.filter((n: string) => this.initialAvailAll.includes(n));
          const pUke = this.pick(receptionOnly, receptionOnly, Math.ceil(needUke), "受付", cUke);
          pUke.forEach((n: string) => { let t = this.getWorkTagForStaff(n); cUke.push(`${n}${t}`); this.addUsage(n, t?0.5:1); this.blockMap.set(n, 'ALL'); });
        }
        ...
        if (!((this.ctx.customRules.linkedRooms || []).some((r: any) => r.target === room)) && !hasPF) { this.fill(cand, room, pList, targetCount, phTags.length > 0 ? phTags[phTags.length - 1] : ""); absenceHelpTags.forEach((helpTag, idx) => { this.fill(cand, room, pList, targetCount + idx + 1, helpTag); }); }
      }
    });
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number, forcedNeedTag: string = "") {
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const remaining = eff.cap - currentAmount; let curAm = eff.amClosed ? 999 : 0; let curPm = eff.pmClosed ? 999 : 0; let placeholderTag = ""; current.forEach(x => { const xc = extractStaffName(x); const xt = x.substring(xc.length); if (ROLE_PLACEHOLDERS.includes(xc)) { if (xt) placeholderTag = xt; return; } if (coversAMWithTag(xt)) curAm++; if (coversPMWithTag(xt)) curPm++; });
      ...
      const getFilterReason = (name: string): RejectReason | null => { if (current.map(extractStaffName).includes(name)) return { hard: true, msg: "同室配置済" }; if (this.isUsed(name)) return { hard: true, msg: "他業務配置済" }; if (this.isForbidden(name, section)) return { hard: true, msg: "担当不可" }; if (section === "MMG" && !this.isMmgCapable(name)) return { hard: true, msg: "MMG外" }; if (!this.canAddKenmu(name, section)) return { hard: true, msg: "兼務上限" }; const b = this.blockMap.get(name); if (needTag && b === 'NONE') { const isSimpleHalf = needTag === '(AM)' || needTag === '(PM)'; if (isSimpleHalf && !eff.pmClosed && !eff.amClosed && !isMonthlyMainStaff(section, name, this.ctx.monthlyAssign)) return { hard: true, msg: "半端枠" }; } if (b === 'ALL') return { hard: true, msg: "全日ブ" }; if (needTag) { const workTag = this.getWorkTagForStaff(name); if (workTag && !tagContains(workTag, needTag)) return { hard: true, msg: "時間不足" }; } if (needTag === "(AM)" && b === 'AM') return { hard: true, msg: "AMブ" }; if (needTag === "(PM)" && b === 'PM') return { hard: true, msg: "PMブ" }; if (eff.pmClosed && b === 'AM') return { hard: true, msg: "午後休" }; if (eff.amClosed && b === 'PM') return { hard: true, msg: "午前休" }; if (this.isTimeTagBlockedByFullDayRule(section, needTag || b || "")) return { hard: true, msg: "終日専任" }; if (this.isHalfDayBlocked(name, section).hard) return { hard: true, msg: "終日専任" }; if (this.isHardNoConsecutive(name, section)) return { hard: false, msg: "連日禁止" }; if (this.hasNGPair(name, current.map(extractStaffName), false)) return { hard: true, msg: "絶対NG" }; if (this.hasNGPair(name, current.map(extractStaffName), true)) return { hard: false, msg: "なるべくNG" }; return null; };
      ...
      const c = pC[0]; const block = this.blockMap.get(c); let t = "", f = 1; if (block === 'AM') { t = needTag && tagContains(this.getWorkTagForStaff(c) || "(PM)", needTag) ? needTag : (this.getWorkTagForStaff(c) || "(PM)"); f = 0.5; this.blockMap.set(c, 'ALL'); this.timeTagMap.delete(c); } else if (block === 'PM') { t = needTag && tagContains(this.getWorkTagForStaff(c) || "(AM)", needTag) ? needTag : (this.getWorkTagForStaff(c) || "(AM)"); f = 0.5; this.blockMap.set(c, 'ALL'); this.timeTagMap.delete(c); } else { if (needTag) { t = needTag; f = 0.5; this.consumeAvailabilityAfterAssignment(c, needTag); } else if (eff.pmClosed) { t = "(AM)"; f = 0.5; this.blockMap.set(c, 'AM'); this.timeTagMap.delete(c); } else if (eff.amClosed) { t = "(PM)"; f = 0.5; this.blockMap.set(c, 'PM'); this.timeTagMap.delete(c); } else { t = ""; f = 1; this.blockMap.set(c, 'ALL'); this.timeTagMap.delete(c); } }
      current.push(`${c}${t}`); if (t) this.ensureSourceRoomBackfill(c, t, section); this.addUsage(c, f); this.log(`✅ [配置決定] ${section} に ${c}${t} ${fM}`);
    }
  ...
  processPostTasks() {
    ...
    if (split(this.dayCells["受付"]).reduce((s, m) => s + getStaffAmount(m), 0) < uT && !this.skipSections.includes("受付ヘルプ")) {
      let hm = split(this.dayCells["受付ヘルプ"]);
      if (hm.length === 0) {
        const generalSourceRooms = ["検像","骨塩","パノラマCT","1号室","2号室","3号室","5号室","透視（6号）","透視（11号）","ポータブル","DSA"];
        const pickReceptionHelp = (exclude: string[], preferKenza = false) => {
          const used = new Set([...exclude, ...hm.map(extractStaffName)]);
          const fromRooms = (preferKenza ? ["検像"] : generalSourceRooms)
            .flatMap((room: string) => split(this.dayCells[room] || "").map(extractStaffName))
            .filter((name: string) => !used.has(name) && !this.isForbidden(name, "受付ヘルプ") && !this.hasNGPair(name, hm.map(extractStaffName), false));
          const uniqRoomCandidates = Array.from(new Set(fromRooms));
          if (uniqRoomCandidates.length > 0) {
            uniqRoomCandidates.sort((a, b) => (this.assignCounts[a] || 0) - (this.assignCounts[b] || 0) || this.getTodayRoomCount(a) - this.getTodayRoomCount(b) || a.localeCompare(b, 'ja'));
            return uniqRoomCandidates[0];
          }
          let fallback = this.initialAvailGeneral.filter((n: string) => !used.has(n) && !this.isForbidden(n, "受付ヘルプ") && !this.hasNGPair(n, hm.map(extractStaffName), false));
          if (fallback.length > 0) {
            fallback.sort((a, b) => (this.assignCounts[a] || 0) - (this.assignCounts[b] || 0) || a.localeCompare(b, 'ja'));
            return fallback[0];
          }
          return null;
        };
        const lC = split(this.dayCells["昼当番"]).map(extractStaffName);
        const midHelp = pickReceptionHelp(lC);
        if (midHelp) {
          hm.push(`${midHelp}(11:30〜)`);
          this.ensureSourceRoomBackfill(midHelp, "(11:30〜)", "受付ヘルプ");
        }
        const lateHelp = pickReceptionHelp(midHelp ? [midHelp] : [], true) || pickReceptionHelp(midHelp ? [midHelp] : []);
        if (lateHelp) {
          hm.push(`${lateHelp}(16:00〜)`);
          this.ensureSourceRoomBackfill(lateHelp, "(16:00〜)", "受付ヘルプ");
        }
      }
      this.dayCells["受付ヘルプ"] = join(hm);
    }
  }
}
             ...
             {RENDER_GROUPS.map((group: RenderGroup) => (
               <div key={group.title} style={{ gridColumn: "1 / -1" }}>
                 ...
                 <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
                   {group.sections.map((s: string) => <SectionEditor key={s} section={s} value={allDays[sel]?.[s] || ""} activeStaff={allStaff} onChange={(v: string) => updateDay(s, v)} noTime={REST_SECTIONS.includes(s) || s === "昼当番"} customOptions={ROLE_PLACEHOLDERS.filter(p => p.startsWith(s))} onAddHelp={s === "不在" ? (staffName: string, fromTime: string) => {
                     const cells = allDays[sel] || {};
                     const rooms = ["CT","MRI","RI","治療","1号室","2号室","3号室","5号室","透視（6号）","透視（11号）","MMG","骨塩","パノラマCT","ポータブル","DSA","検像"];
                     const core = extractStaffName(staffName);
                     const found = rooms.find(r => split(cells[r]||"").some((m:string)=>extractStaffName(m)===core));
                     const helpMap = { ...parseAbsenceHelpMap(cells) };
                     if (fromTime === "") delete helpMap[core];
                     else if (fromTime === ABSENCE_HELP_NONE) helpMap[core] = ABSENCE_HELP_NONE;
                     else helpMap[core] = fromTime;
                     updateDay(ABSENCE_HELP_KEY, Object.keys(helpMap).length > 0 ? JSON.stringify(helpMap) : "");
                     if (found) {
                       const prefix = found + "枠";
                       const nextEntries = split(cells[found] || "").filter((m:string) => !m.startsWith(prefix));
                       if (fromTime && fromTime !== ABSENCE_HELP_NONE) nextEntries.push(prefix + fromTime);
                       updateDay(found, join(nextEntries));
                     }
                   } : undefined} dayCells={s === "不在" ? (allDays[sel] || {}) : undefined} />)}
                 </div>
               </div>
             ))}
          </div>
