...
const KEY_ALL_DAYS = "shifto_alldays_v101"; // キャッシュ破棄用のマイグレーション対策
const KEY_MONTHLY = "shifto_monthly_v101"; 
const KEY_RULES = "shifto_rules_v101";

const TIME_OPTIONS: string[] = ["(AM)", "(PM)", "(12:15〜13:00)", "(17:00〜19:00)", "(17:00〜22:00)"];
for (let h = 8; h <= 19; h++) {
  for (let m = 0; m < 60; m += 15) {
    // 🌟 追加: 8:00は始業時間であり、シフトの途中入場（遅番など）の区切りとしては不要なためスキップ
    if (h === 8 && m === 0) continue; 
    const mm = m === 0 ? "00" : m;
  ...
  return (
    <div style={{ padding: "20px 12px", maxWidth: 1400, margin: "0 auto", background: "#f4f7f9" }}>
        ...
        <div style={{ ...panelStyle(), padding: "16px 20px" }}>
          <details>
            ...
            <div style={{ paddingTop: 20, borderTop: "2px dashed #e2e8f0", marginTop: 16, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(350px, 1fr))", gap: 24 }}>
              ...
              <div style={{ background: "#fef08a", padding: 16, borderRadius: 12, border: "1px solid #fde047" }}>
      ...
      <div className="print-area" style={{ ...panelStyle(), marginBottom: 24, padding: "20px 12px" }}>
        ...
        {/* 🌟 改善: テーブルのスクロール設定を修正し、ページスクロールでStickyが効くように変更 */}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800, background: "#fff" }}>
            ...
            <tbody>
              {SECTIONS.map((section, index) => {
                const isZebra = index % 2 === 1; // 🌟 改善: ゼブラストライプ
                return (
                  <tr key={section}>
