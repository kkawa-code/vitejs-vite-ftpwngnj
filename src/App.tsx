...
const KEY_ALL_DAYS = "shifto_alldays_v103"; 
const KEY_MONTHLY = "shifto_monthly_v103"; 
const KEY_RULES = "shifto_rules_v103";
...
for (let h = 8; h <= 19; h++) {
  for (let m = 0; m < 60; m += 15) {
    // 8:00は始業時間であり、シフトの途中入場（遅番など）の区切りとしては不要なためスキップ
    if (h === 8 && m === 0) continue; 
    const mm = pad(m);
...
const isMonthlyMainStaff = (section: string, name: string, monthlyAssign: Record<string, string>) => {
  ...
  if (section === "治療") return split(monthlyAssign.治療 || "").includes(name);
  if (section === "RI") return split(monthlyAssign.RI || "").includes(name);
  if (section === "CT") return split(monthlyAssign.CT || "").includes(name);
  if (section === "MRI") return split(monthlyAssign.MRI || "").includes(name);
  if (section === "MMG") return split(monthlyAssign.MMG || "").includes(name);
  if (section === "透析後胸部") return split(monthlyAssign.透析後胸部 || "").includes(name);
  if (section === "受付") return split(monthlyAssign.受付 || "").includes(name);
  return split(monthlyAssign[section] || "").includes(name);
};
  ...
  return (
    <div style={{ padding: "20px 12px", maxWidth: 1400, margin: "0 auto", background: "#f4f7f9" }}>
      ...
      <div className="no-print" style={{ display: "grid", gap: 16, marginBottom: 24 }}>
                    ...
                    {priorityRoomsList.map((room: string, idx: number, arr: string[]) => (
                      <div key={room} style={{ display: "flex", alignItems: "center", background: "#fff", padding: "4px 8px", borderRadius: 8, border: "1px solid #fcd34d", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                          ...
                          }} disabled={idx === arr.length - 1} style={{ border: "none", background: idx === arr.length - 1 ? "transparent" : "#fef3c7", cursor: idx === arr.length - 1 ? "default" : "pointer", fontSize: 10, padding: "2px 6px", borderRadius: 4, color: "#92400e", lineHeight: 1 }}>▼</button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ ...panelStyle(), padding: "16px 20px" }}>
          <details>
            <summary style={{ fontWeight: 800, color: "#4338ca", padding: "4px", fontSize: 16, display: "flex", alignItems: "center", gap: 8, letterSpacing: "0.02em" }}>
              <span>🍱</span> ② 日々の業務ルール（昼当番・遅番・サポート・常時兼務）
            </summary>
            <div style={{ paddingTop: 20, borderTop: "2px dashed #e2e8f0", marginTop: 16, display: "grid", gap: 24 }}>
              
              {/* 🌟 常時兼務ペア追加 */}
              <div style={{ background: "#ecfdf5", padding: 16, borderRadius: 12, border: "1px solid #a7f3d0" }}>
                <h4 style={{ margin: "0 0 8px 0", color: "#065f46", fontSize: 14, fontWeight: 800 }}>🔗 常時兼務ペア</h4>
                <p style={{ fontSize: 12, color: "#047857", marginBottom: 12, fontWeight: 600 }}>
                  人が足りない時に自動で兼務にする部屋のペアです。余裕がある時は独立した担当者が入ります。
                </p>
                {(customRules.kenmuPairs || []).map((rule: any, idx: number) => (
                  <div key={idx} className="rule-row" style={{ background: "#fff", padding: "8px 12px", border: "1px solid #a7f3d0", borderRadius: 8 }}>
              ...
              <div style={{ background: "#eef2ff", padding: 16, borderRadius: 12, border: "1px solid #c7d2fe" }}>
                ...
                <p style={{ fontSize: 12, color: "#4f46e5", marginBottom: 12, fontWeight: 600 }}>※RI担当者は、NG設定されていない限り最優先で昼当番にアサインされます。</p>
        ...
        <div style={{ ...panelStyle(), padding: "16px 20px" }}>
          <details>
            <summary style={{ fontWeight: 800, color: "#b91c1c", padding: "4px", fontSize: 16, display: "flex", alignItems: "center", gap: 8, letterSpacing: "0.02em" }}>
              <span>🚨</span> ③ イレギュラー対応（玉突き・代打・NG・緊急など）
            </summary>
            <div style={{ paddingTop: 20, borderTop: "2px dashed #e2e8f0", marginTop: 16, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(350px, 1fr))", gap: 24 }}>
              
              <div style={{ background: "#e0f2fe", padding: 16, borderRadius: 12, border: "1px solid #bae6fd" }}>
      ...
      <div className="no-print" style={{ ...panelStyle(), borderRadius: "24px 24px 0 0", boxShadow: "0 -4px 20px rgba(0,0,0,0.03)" }}>
        ...
        <div className="scroll-container hide-scrollbar sticky-header" style={{ display: "flex", gap: 10, borderBottom: "2px solid #e2e8f0", paddingBottom: 12, marginBottom: 20, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0, alignItems: "center" }}>
            ...
            <button className="btn-hover" onClick={handleExport} style={{...btnStyle("#6366f1"), padding: "8px 14px", fontSize: 13}}>💾 保存</button>
