...
export const KEY_ALL_DAYS = "shifto_alldays_v295"; export const KEY_MONTHLY = "shifto_monthly_v295"; export const KEY_RULES = "shifto_rules_v295";
...
export const WeekCalendarPicker = ({ targetMonday, onChange, nationalHolidays, customHolidays }: any) => {
  ...
  const handleWeekClick = (weekObj: (number|null)[]) => {
    let validDay = weekObj.find(d => d !== null);
    if (!validDay) return;
        ...
        <>
          ...
          <div style={{ position: "absolute", top: "110%", left: 0, background: "#fff", border: "1px solid #cbd5e1", borderRadius: 12, padding: 20, zIndex: 50, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.15)", minWidth: 320 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "center", fontSize: 16 }}>
              <thead>
                <tr><th style={{ color: "#ef4444", padding: 8 }}>日</th><th>月</th><th>火</th><th>水</th><th>木</th><th>金</th><th style={{ color: "#3b82f6", padding: 8 }}>土</th></tr>
              </thead>
              <tbody>
                {weeks.map((w, wIdx) => {
...
export class AutoAssigner {
  ...
  isFDO(r: string) { return split(this.ctx.customRules.fullDayOnlyRooms || "").includes(r); }
  ...
  execute(): DayData {
    ...
    if (this.isSmartFix) {
      ...
      priority.forEach((room: string) => {
         ...
         const cap = this.dynamicCapacity[room] || 1; const eff = this.getEffectiveTarget(room, cap); if (eff.al) return;
         ...
         while (getAmt(current) < eff.cap) {
            const freeStaff = sortedAvail.find((s: string) => { 
                ...
                const b = this.blockMap.get(s); if (eff.pC && b === 'AM') return false; if (eff.aC && b === 'PM') return false; return true; 
            });
            if (!freeStaff) break;
            const block = this.blockMap.get(freeStaff); let tag = ""; let p = 1; 
            if (block === 'AM') { tag = "(PM)"; p = 0.5; } else if (block === 'PM') { tag = "(AM)"; p = 0.5; } else { if (eff.pC) { tag = "(AM)"; p = 0.5; } else if (eff.aC) { tag = "(PM)"; p = 0.5; } }
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number) {
    ...
    const eff = this.gET(section, targetCount); if (eff.al) return;
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const remaining = eff.cap - currentAmount; let curAm = eff.aC ? 999 : 0; let curPm = eff.pC ? 999 : 0; current.forEach(x => { if (x.includes("(AM)")) curAm++; else if (x.includes("(PM)")) curPm++; else { curAm++; curPm++; } });
      ...
      const getFilterReason = (name: string): RejectReason | null => { if (current.map(extractStaffName).includes(name)) return { hard: true, msg: "同室配置済" }; if (this.isUsed(name)) return { hard: true, msg: "他業務配置済" }; if (this.isForbidden(name, section)) return { hard: true, msg: "担当不可" }; if (section === "MMG" && !this.isMmgCapable(name)) return { hard: true, msg: "MMG外" }; if (!this.canAddKenmu(name, section)) return { hard: true, msg: "兼務上限" }; const b = this.blockMap.get(name); if (needTag && b === 'NONE') { if (!eff.pC && !eff.aC && !isMonthlyMainStaff(section, name, this.ctx.monthlyAssign)) return { hard: true, msg: "半端枠" }; } if (b === 'ALL') return { hard: true, msg: "全日ブ" }; if (needTag === "(AM)" && b === 'AM') return { hard: true, msg: "AMブ" }; if (needTag === "(PM)" && b === 'PM') return { hard: true, msg: "PMブ" }; if (eff.pC && b === 'AM') return { hard: true, msg: "午後休" }; if (eff.aC && b === 'PM') return { hard: true, msg: "午前休" }; if (this.noHD(section, needTag || b || "")) return { hard: true, msg: "終日専任" }; if (this.iHB(name, section).h) return { hard: true, msg: "終日専任" }; if (this.iHNC(name, section)) return { hard: false, msg: "連日禁止" }; if (this.hasNGPair(name, current.map(extractStaffName), false)) return { hard: true, msg: "絶対NG" }; if (this.hasNGPair(name, current.map(extractStaffName), true)) return { hard: false, msg: "なるべくNG" }; return null; };
  ...
  processPostTasks() {
    ...
    (this.ctx.customRules.smartKenmu||[]).forEach((r:any)=>{
      ...
      for(const tm of tM){const tc=extractStaffName(tm); if(ROLE_PLACEHOLDERS.includes(tc)||ROOM_SECTIONS.some(rm=>rm!==r.targetRoom&&split(this.dc[rm]).map(extractStaffName).includes(tc)))continue; 
    ...
    (this.cx.customRules.linkedRooms||[]).forEach((r:any)=>{
      ...
      for(const sSR of split(r.sources)){if(cA>=tC&&cP>=tC)break; const {r:sR,min}=parseRoomCond(sSR); if(sR==="透析後胸部"||(min>0&&split(this.dc[sR]).reduce((s,m)=>s+getStaffAmount(m),0)<min))continue;
        split(this.dc[sR]).forEach(m=>{if(cA>=tC&&cP>=tC)return; const c=extractStaffName(m); if(!ROLE_PLACEHOLDERS.includes(c)&&!cM.map(extractStaffName).includes(c)&&!this.iFb(c,r.target)&&!this.iHB(c,r.target).h&&!this.hNG(c,cM.map(extractStaffName),false)&&!this.iHNC(c,r.target)&&(r.target==="MMG"?this.iMC(c):true)&&this.cAK(c,r.target,true)&&!m.includes("17:")&&!m.includes("19:")&&!this.noHD(r.target,m)){let pS=m; if(r.target==="パノラマCT"&&sR==="透視（6号）"){if(m.includes("(PM)"))return; pS=`${c}(AM)`;}else{if(cA<tC&&cP>=tC){if(m.includes("(PM)"))return; pS=`${c}(AM)`;}else if(cA>=tC&&cP<tC){if(m.includes("(AM)"))return; pS=`${c}(PM)`;}else if(e.pC){if(m.includes("(PM)"))return; pS=`${c}(AM)`;}else if(e.aC){if(m.includes("(AM)"))return; pS=`${c}(PM)`;}} cM.push(pS); if(pS.includes("(AM)"))cA++;else if(pS.includes("(PM)"))cP++;else{cA++;cP++;} this.aU(c,getStaffAmount(pS)); this.uBM(c,pS); this.lg(`🔗 [基本兼務] ${sR} の ${pS} を ${r.target} に配置`);}});
    ...
    if(r2A<2&&(nAH||nPH)){
      ...
      if(nAH){let p=fC(true,["1号室","5号室"])??fC(true,[]); if(p){this.dc["2号室"]=join([...split(this.dc["2号室"]),`${p}(AM)`]); this.addU(p,0.5); this.bm.set(p,this.bm.get(p)==='PM'?'ALL':'AM'); this.log(`🤝 [ポータブル特例] 2号室のポータブル兼務(AM)のため ${p} を追加`);}}
      if(nPH){let p=fC(false,["1号室","5号室"])??fC(false,[]); if(p){this.dc["2号室"]=join([...split(this.dc["2号室"]),`${p}(PM)`]); this.addU(p,0.5); this.bm.set(p,this.bm.get(p)==='AM'?'ALL':'PM'); this.log(`🤝 [ポータブル特例] 2号室のポータブル兼務(PM)のため ${p} を追加`);}}
    }
    ...
    if(!this.skipSections.includes("昼当番")){
      let cL=split(this.dc["昼当番"]); let lT=this.ctx.customRules.lunchBaseCount??3; const dC=this.day.label.match(/\((.*?)\)/)?.[1]; if(dC){const sd=(this.ctx.customRules.lunchSpecialDays||[]).find((x:any)=>x.day===dC); if(sd)lT=Number(sd.count);}
      (this.ctx.customRules.lunchRoleRules||[]).forEach((r:any)=>{if(r.day==="毎日"||r.day===dC){const rS=split(this.ctx.monthlyAssign[r.role]||"").map(extractStaffName), tM=split(this.dc[r.role]||"").map(extractStaffName); let sl:string|null=null; for(const src of split(r.sourceRooms)){const{r:sR,min}=parseRoomCond(src); const rM=split(this.dc[sR]||""), rq=min>0?min:(this.cap[sR]||1); if(rM.reduce((su,m)=>su+getStaffAmount(m),0)>=rq){sl=rM.map(extractStaffName).find(n=>rS.includes(n)&&!tM.includes(n)&&!this.iFb(n,"昼当番")&&!this.hNG(n,cL,false))||null;} if(sl)break;} if(!sl)sl=tM.find(n=>!this.iFb(n,"昼当番")&&!this.hNG(n,cL,false))||null; if(sl&&!cL.includes(sl)&&cL.length<lT)cL.push(sl);}});
      ...
      split(this.ctx.customRules.lunchPrioritySections??"RI,1号室,2号室,3号室,5号室,CT").forEach(sc=>split(this.dc[sc]).forEach(n=>{const c=extractStaffName(n);if(!cL.includes(c)&&cL.length<lT&&!this.iFb(c,"昼当番")&&!this.hNG(c,cL,false))cL.push(c);}));
      if(cL.length<lT){(this.ctx.customRules.lunchConditional||[]).forEach((co:any)=>{const sM=split(this.dc[co.section]); if(sM.length>=Number(co.min)){let p=0; for(const n of sM){if(p>=Number(co.out)||cL.length>=lT)break; const c=extractStaffName(n); if(!cL.includes(c)&&!this.iFb(c,"昼当番")&&!this.hNG(c,cL,false)){cL.push(c);p++;}}}});}
      if(cL.length<lT){const lrM:string[]=[]; split(this.ctx.customRules.lunchLastResortSections??"治療").forEach(sc=>split(this.dc[sc]).forEach(n=>lrM.push(extractStaffName(n)))); this.iaGen.filter((n:string)=>!lrM.includes(n)&&!cL.includes(n)&&!this.iFb(n,"昼当番")&&!this.hNG(n,cL,false)).forEach((n:string)=>{if(cL.length<lT)cL.push(n);}); this.iaGen.filter((n:string)=>lrM.includes(n)&&!cL.includes(n)&&!this.iFb(n,"昼当番")&&!this.hNG(n,cL,false)).forEach((n:string)=>{if(cL.length<lT)cL.push(n);});}
...
export default function App(): any {
  ...
  const getDailyStats = (dayId: string) => { 
    ...
    let restMems: string[] = []; REST_SECTIONS.forEach(rs => { restMems.push(...split(cells[rs] || "")); });
    const workingStaff = allStaff.filter(s => { 
      const isFullRest = restMems.some(m => extractStaffName(m) === s && !m.includes("(AM)") && !m.includes("(PM)")); 
      return !isFullRest; 
    }); 
  ...
  return (
    <div style={{ maxWidth: "98%", margin: "0 auto", padding: "24px", boxSizing: "border-box" }}>
      ...
      <div className="no-print" style={{ ...panelStyle(), display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, padding: "20px 32px", background: "linear-gradient(to right, #ffffff, #f8fafc)" }}>
        <h2 style={{ margin: 0, color: "#0f172a", fontSize: 26, fontWeight: 900 }}>勤務割付システム Ver 2.95</h2>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <Btn oC={() => setTargetMonday((prev:any) => { const d=new Date(prev); d.setDate(d.getDate()-7); return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`; })} bg="#f1f5f9" c="#475569" ch="◀ 先週" />
          <WeekCalendarPicker targetMonday={targetMonday} onChange={setTargetMonday} nationalHolidays={nationalHolidays} customHolidays={customHolidays} />
          <Btn oC={() => setTargetMonday((prev:any) => { const d=new Date(prev); d.setDate(d.getDate()+7); return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`; })} bg="#f1f5f9" c="#475569" ch="来週 ▶" />
        </div>
      </div>
      ...
      <div className="no-print" style={{ display: activeTab === 'rules' ? 'block' : 'none' }}>
        ...
        <div style={{ ...panelStyle() }}>
          ...
          <div style={{ borderLeft: "8px solid #94a3b8", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#f8fafc" border="#cbd5e1" color="#334155" icon="🙅" title="担当不可ルール" ch={
              <>{(customRules.forbidden || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 16, borderBottom: "1px solid #e2e8f0", paddingBottom: 16 }}>
                    <Row><StaffSel v={rule.staff} oC={(v:any)=>updateRule("forbidden", idx, "staff", v)} list={activeGeneralStaff} /><DelBtn oC={()=>removeRule("forbidden", idx)} /></Row>
                    <MultiPicker selected={rule.sections} oC={(v: string) => updateRule("forbidden", idx, "sections", v)} opt={ASSIGNABLE_SECTIONS} />
                  </div>
              ))}<button className="rule-add" style={{color:"#475569", borderColor:"#cbd5e1"}} onClick={() => addRule("forbidden", { staff: "", sections: "" })}>＋ 追加</button></>
            } />

            <RuleCard bg="#f0fdf4" border="#bbf7d0" color="#15803d" icon="🔒" title="専従ルール" ch={
              <>{(customRules.fixed || []).map((rule: any, idx: number) => (
                  <Row key={idx}><StaffSel v={rule.staff} oC={(v:any)=>updateRule("fixed", idx, "staff", v)} list={activeGeneralStaff} /><RoomSel v={rule.section} oC={(v:any)=>updateRule("fixed", idx, "section", v)} list={ROOM_SECTIONS} /><DelBtn oC={()=>removeRule("fixed", idx)} /></Row>
              ))}<button className="rule-add" style={{color:"#15803d", borderColor:"#86efac"}} onClick={() => addRule("fixed", { staff: "", section: "" })}>＋ 追加</button></>
            } />

            <RuleCard bg="#fef2f2" border="#fecaca" color="#b91c1c" icon="🚫" title="NGペア" ch={
              <>{(customRules.ngPairs || []).map((rule: any, idx: number) => (
                  <Row key={idx}><StaffSel v={rule.s1} oC={(v:any)=>updateRule("ngPairs", idx, "s1", v)} list={activeGeneralStaff} /><span className="rule-label">と</span><StaffSel v={rule.s2} oC={(v:any)=>updateRule("ngPairs", idx, "s2", v)} list={activeGeneralStaff} /><select value={rule.level || "hard"} onChange={(e: any) => updateRule("ngPairs", idx, "level", e.target.value)} className="rule-sel" style={{borderColor:"#fca5a5", color:"#b91c1c", flex:"0 0 auto", minWidth:"120px"}}><option value="hard">絶対NG</option><option value="soft">なるべくNG</option></select><DelBtn oC={()=>removeRule("ngPairs", idx)} /></Row>
              ))}<button className="rule-add" style={{color:"#b91c1c", borderColor:"#fca5a5"}} onClick={() => addRule("ngPairs", { s1: "", s2: "", level: "hard" })}>＋ NGペアを追加</button></>
            } />

            <RuleCard bg="#f8fafc" border="#cbd5e1" color="#334155" icon="🕒" title="終日専任・連日禁止" ch={
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 260 }}><label style={{ fontSize: 17, fontWeight: 800, color: "#475569", display: "block", marginBottom: 8 }}>【終日専任】半休・AM/PM不可の部屋</label><MultiPicker selected={customRules.fullDayOnlyRooms ?? ""} oC={(v: string) => setCustomRules({...customRules, fullDayOnlyRooms: v})} opt={ROOM_SECTIONS} /></div>
                <div style={{ flex: 1, minWidth: 260 }}><label style={{ fontSize: 17, fontWeight: 800, color: "#475569", display: "block", marginBottom: 8 }}>【連日禁止】2日連続で担当させない部屋</label><MultiPicker selected={customRules.noConsecutiveRooms ?? ""} oC={(v: string) => setCustomRules({...customRules, noConsecutiveRooms: v})} opt={ROOM_SECTIONS} /></div>
              </div>
            } />

            <RuleCard bg="#fdf4ff" border="#f5d0fe" color="#86198f" icon="🏠" title="遅番不可スタッフ・部屋" ch={
              <><label style={{ fontSize: 17, fontWeight: 800, color: "#86198f", display: "block", marginBottom: 8 }}>遅番不可スタッフ</label>
              <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "2px solid #f0abfc", minHeight: "56px", display: "flex", alignItems: "center", marginBottom: 16 }}><MultiPicker selected={customRules.noLateShiftStaff || ""} oC={(v: string) => setCustomRules({...customRules, noLateShiftStaff: v})} opt={allStaff} ph="＋スタッフを選択" /></div>
              <label style={{ fontSize: 17, fontWeight: 800, color: "#86198f", display: "block", marginBottom: 8 }}>その日その部屋に入っている人も遅番除外</label>
              <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "2px solid #f0abfc", minHeight: "56px", display: "flex", alignItems: "center" }}><MultiPicker selected={customRules.noLateShiftRooms || ""} oC={(v: string) => setCustomRules({...customRules, noLateShiftRooms: v})} opt={ROOM_SECTIONS} ph="＋部屋を選択" /></div></>
            } />
          </div>
          ...
          <div style={{ borderLeft: "8px solid #f59e0b", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fef08a" border="#fde047" color="#a16207" icon="🚨" title="緊急ルール（人数不足時）" ch={
              ...
              {(customRules.emergencies || []).map((em: any, idx: number) => (
                  <div key={idx} style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginBottom: '12px', background: '#fff', padding: '16px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                    <div style={{ flex: '1 1 350px', display: 'flex', gap: '8px', alignItems: 'center', borderRight: '2px dashed #cbd5e1', paddingRight: '16px' }}>
                      ...
                      {em.type !== 'empty_room_swap' && <><NumInp v={em.threshold || 0} oC={(v:any)=>updateRule("emergencies", idx, "threshold", v)} w={60} />人以下</>}
                    ...
                    <div style={{ flex: '1 1 400px', display: 'flex', gap: '8px', alignItems: 'center', paddingLeft: '8px' }}>
                       ...
                       {em.type === 'change_capacity' && <><RoomSel v={em.section} oC={(v:any)=>updateRule("emergencies", idx, "section", v)} list={ROOM_SECTIONS} /> の定員を <NumInp v={em.newCapacity||1} oC={(v:any)=>updateRule("emergencies", idx, "newCapacity", v)} w={60} /> 名にする</>}
                    ...
                    <DelBtn oC={()=>removeRule("emergencies", idx)} />
                  </div>
              ))}
            ...
            <RuleCard bg="#f0fdf4" border="#bbf7d0" color="#15803d" icon="🔄" title="メイン配置の交換ルール" ch={
              <>{(customRules.swapRules || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 20, borderBottom: "2px solid #bbf7d0", paddingBottom: 20 }}>
                    <div className="rule-row">
                      ...
                      <DelBtn oC={()=>removeRule("swapRules", idx)} />
                    </div>
                    <div className="rule-row">
                      <span className="rule-label" style={{color:"#15803d", fontSize: 17}}>以下の部屋の担当者とメイン配置を交換する（※左の部屋から優先）:</span>
                      <MultiPicker selected={rule.sourceRooms} oC={(v: string) => updateRule("swapRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} />
                    </div>
                  </div>
            ...
            <RuleCard bg="#fff7ed" border="#fed7aa" color="#c2410c" icon="🔄" title="代打ルール" ch={
              <>{(customRules.substitutes || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 16, alignItems: "center", background: "#fff", padding: "16px", borderRadius: 8, border: "1px solid #fdba74", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
                    <div style={{ flex: 1, minWidth: "200px" }}><MultiPicker selected={rule.target} oC={(v: string) => updateRule("substitutes", idx, "target", v)} opt={activeGeneralStaff} ph="対象スタッフ(休)" /></div>
                    <span style={{ fontSize: 17, fontWeight: 800, color: "#c2410c" }}>が全員休みの時➔</span>
                    <div style={{ flex: 1, minWidth: "200px" }}><MultiPicker selected={rule.subs} oC={(v: string) => updateRule("substitutes", idx, "subs", v)} opt={activeGeneralStaff} ph="代打スタッフを追加" /></div>
                    ...
                    <DelBtn oC={()=>removeRule("substitutes", idx)} />
                  </div>
            ...
            <RuleCard bg="#e0f2fe" border="#bae6fd" color="#0369a1" icon="🎱" title="玉突きルール" ch={
              <>{(customRules.pushOuts || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 20, borderBottom: "1px solid #bae6fd", paddingBottom: 20 }}>
                    <div className="rule-row">
                      ...
                      <DelBtn oC={()=>removeRule("pushOuts", idx)} />
                    </div>
                    <div className="rule-row">
                      <span className="rule-label" style={{color:"#0284c7"}}>以下の部屋に移動（※左から優先）:</span>
                      <MultiPicker selected={rule.targetSections} oC={(v: string) => updateRule("pushOuts", idx, "targetSections", v)} opt={ROOM_SECTIONS} />
                    </div>
                  </div>
          ...
          <div style={{ borderLeft: "8px solid #3b82f6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fffbeb" border="#fde68a" color="#b45309" icon="👑" title="部屋の割り当て優先順位" ch={
            ...
            <RuleCard bg="#f8fafc" border="#cbd5e1" color="#334155" icon="👥" title="絶対優先の定員設定" ch={
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                {Object.entries(customRules.capacity || {}).map(([room, count]) => (
                  <div key={room} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", padding: "10px 16px", borderRadius: 8, border: "1px solid #cbd5e1" }}>
                    ...
                    <NumInp v={count} oC={(v:any)=>setCustomRules({...customRules, capacity: {...customRules.capacity, [room]: v}})} w={60} />
            ...
            <RuleCard bg="#fff" border="#e2e8f0" color="#334155" icon="📅" title="月間担当者の設定" ch={
          ...
          <div style={{ borderLeft: "8px solid #10b981", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#fdf4ff" border="#f0abfc" color="#86198f" icon="✨" title="スマート兼務（専任担当の負担軽減・引き抜き）" ch={
              <>{(customRules.smartKenmu || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ background: "#fff", padding: "16px 20px", border: "1px solid #f0abfc", borderRadius: 8, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 6 }}><MultiPicker selected={rule.sourceRooms} oC={(v: string) => updateRule("smartKenmu", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
                    <DelBtn oC={()=>removeRule("smartKenmu", idx)} />
                  </div>
            ...
            <RuleCard bg="#ecfdf5" border="#a7f3d0" color="#065f46" icon="🔗" title="兼務・セット配置ルール" ch={
              ...
              {(customRules.kenmuPairs || []).map((rule: any, idx: number) => (
                <div key={idx} className="rule-row" style={{ background: "#fff", padding: "12px 16px", border: "1px solid #a7f3d0", borderRadius: 8 }}>
                  ...
                  <DelBtn oC={()=>removeRule("kenmuPairs", idx)} />
                </div>
              ))}
              ...
              {(customRules.linkedRooms || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #a7f3d0", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10, marginBottom: 10 }}><MultiPicker selected={rule.sources} oC={(v: string) => updateRule("linkedRooms", idx, "sources", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flexShrink: 0, alignItems: "center" }}>
                      ...
                      <DelBtn oC={()=>removeRule("linkedRooms", idx)} />
                    </div>
                  </div>
              ))}
              ...
              {(customRules.rescueRules || []).map((rule: any, idx: number, arr: any[]) => (
                  <div key={idx} style={{ background: "#fff", padding: "20px 24px", border: "1px solid #fde047", borderRadius: 12, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginLeft: 20, marginTop: 10 }}><MultiPicker selected={rule.sourceRooms} oC={(v: string) => updateRule("rescueRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} /></div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flexShrink: 0, alignItems: "center" }}>
                      ...
                      <DelBtn oC={()=>removeRule("rescueRules", idx)} />
                    </div>
                  </div>
              ))}
            ...
            <RuleCard bg="#f5f3ff" border="#ddd6fe" color="#6d28d9" icon="🌆" title="遅番ルール" ch={
              <><div style={{ marginBottom: 16, background: "#fff", padding: 16, borderRadius: 8, border: "1px solid #ddd6fe" }}>
                ...
                <MultiPicker selected={customRules.lateShiftLowPriorityStaff || ""} oC={(v: string) => setCustomRules({...customRules, lateShiftLowPriorityStaff: v})} opt={allStaff} ph="＋スタッフを選択" />
              </div>
              {(customRules.lateShifts || []).map((rule: any, idx: number) => (
                  <div key={idx} className="rule-row" style={{background:"#fff", padding:"12px 16px", border:"1px solid #ddd6fe", borderRadius:8}}>
                    ...
                    <DelBtn oC={()=>removeRule("lateShifts", idx)} />
                  </div>
              ))}
            ...
            <RuleCard bg="#fff1f2" border="#fecaca" color="#be185d" icon="⚠️" title="兼務上限のストッパー設定" ch={
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <NumInp v={customRules.alertMaxKenmu ?? 3} oC={(v:any)=>setCustomRules({...customRules, alertMaxKenmu: v})} w={80} />
          ...
          <div style={{ borderLeft: "8px solid #8b5cf6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#eef2ff" border="#c7d2fe" color="#4338ca" icon="🍱" title="昼当番ルール" ch={
              <><div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24, background: "#fff", padding: "12px 20px", borderRadius: 10, border: "1px solid #c7d2fe", width: "fit-content" }}>
                  <span style={{ fontSize: 19, fontWeight: 800, color: "#3730a3" }}>基本の人数:</span><NumInp v={customRules.lunchBaseCount ?? 3} oC={(v:any)=>setCustomRules({...customRules, lunchBaseCount: v})} w={80} />
              </div>
              ...
              <div style={{ background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", marginBottom: 20 }}>
                ...
                {(customRules.lunchRoleRules || []).map((rule: any, idx: number) => (
                  <div key={idx} className="rule-row" style={{ borderBottom: "1px dashed #e0e7ff", paddingBottom: 16, marginBottom: 16 }}>
                    ...
                    <MultiPicker selected={rule.sourceRooms} oC={(v: string) => updateRule("lunchRoleRules", idx, "sourceRooms", v)} opt={EXTENDED_ROOM_SECTIONS} />
                    <DelBtn oC={()=>removeRule("lunchRoleRules", idx)} />
                  </div>
                ))}
                  ...
                  <div style={{ flex: 1, minWidth: "260px", background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff" }}>
                    ...
                    {(customRules.lunchSpecialDays || []).map((rule: any, idx: number) => (
                      <div key={idx} className="rule-row">
                        ...
                        <span className="rule-label">は</span>
                        <NumInp v={rule.count} oC={(v:any)=>updateRule("lunchSpecialDays", idx, "count", v)} />
                        <DelBtn oC={()=>removeRule("lunchSpecialDays", idx)} />
                      </div>
                    ))}
                  ...
                  <div style={{ flex: 1, minWidth: "260px", background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff" }}>
                    ...
                    {(customRules.lunchConditional || []).map((rule: any, idx: number) => (
                      <div key={idx} className="rule-row">
                        ...
                        <NumInp v={rule.min} oC={(v:any)=>updateRule("lunchConditional", idx, "min", v)} />
                        <span className="rule-label">人以上➔</span>
                        <NumInp v={rule.out} oC={(v:any)=>updateRule("lunchConditional", idx, "out", v)} />
                        <DelBtn oC={()=>removeRule("lunchConditional", idx)} />
                      </div>
                    ))}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 20, marginTop: 24 }}>
                  <div style={{ flex: 1, background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", minWidth: "260px" }}>
                    ...
                    <MultiPicker selected={customRules.lunchPrioritySections ?? "RI,1号室,2号室,3号室,5号室,CT"} oC={(v: string) => setCustomRules({...customRules, lunchPrioritySections: v})} opt={ROOM_SECTIONS} />
                  </div>
                  <div style={{ flex: 1, background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e0e7ff", minWidth: "260px" }}>
                    <h6 style={{ margin: "0 0 12px 0", fontSize: 17, color: "#4f46e5", fontWeight: 800 }}>⚠️ 緊急時のみ選出する部屋（なるべく除外）</h6>
                    <MultiPicker selected={customRules.lunchLastResortSections ?? "治療"} oC={(v: string) => setCustomRules({...customRules, lunchLastResortSections: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div></>
            } />

            <RuleCard bg="#f0fdf4" border="#bbf7d0" color="#15803d" icon="🤝" t="サポート専任（2人目要員）ルール" ch={
              <><div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: "260px" }}>
                    ...
                    <div style={{ background: "#fff", padding: "12px", borderRadius: 8, border: "1px solid #86efac", minHeight: "56px", display: "flex", alignItems: "center" }}>
                      <MultiPicker selected={customRules.supportStaffList || ""} oC={(v: string) => setCustomRules({...customRules, supportStaffList: v})} opt={allStaff} ph="＋スタッフを選択" />
                    </div>
                  </div>
                  <div style={{ flex: 2, minWidth: "260px" }}>
                    <label style={{ fontSize: 17, fontWeight: 800, color: "#166534", display: "block", marginBottom: 8 }}>優先する対象部屋</label>
                    <MultiPicker selected={customRules.supportTargetRooms ?? "1号室,2号室,5号室,パノラマCT"} oC={(v: string) => setCustomRules({...customRules, supportTargetRooms: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div>
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start", marginTop: 16 }}>
                  <div style={{ flex: 1, minWidth: "260px" }}>
                    ...
                    <MultiPicker selected={customRules.supportTargetRoomsLowImpact ?? "3号室,パノラマCT"} oC={(v: string) => setCustomRules({...customRules, supportTargetRoomsLowImpact: v})} opt={ROOM_SECTIONS} />
                  </div>
                  <div style={{ flex: 2, minWidth: "260px" }}>
                    <label style={{ fontSize: 15, fontWeight: 800, color: "#166534", display: "block", marginBottom: 4 }}>高影響グループ（極力補充元にしない部屋）</label>
                    <MultiPicker selected={customRules.supportTargetRoomsHighImpact ?? "CT,MRI,治療,RI,ポータブル,2号室,1号室,5号室,透視（6号）,透視（11号）,骨塩,検像"} oC={(v: string) => setCustomRules({...customRules, supportTargetRoomsHighImpact: v})} opt={ROOM_SECTIONS} />
                  </div>
              </div></>
            } />
          </div>
      ...
      {/* ===================== モーダル類 ===================== */}
      {showUnassignedList && (
        <Modal t="未配置のスタッフ" oC={() => setShowUnassignedList(null)} ch={
      ...
      {selectedErrorDay && (
        <Modal t={`👀 ${selectedErrorDay} の確認事項`} oC={() => setSelectedErrorDay(null)} ch={
      ...
      {selectedStaffForStats && (
        <Modal t={`👤 ${selectedStaffForStats} さんの詳細`} oC={() => setSelectedStaffForStats(null)} ch={
      ...
      {showLogDay && (
        <Modal t={`🤔 ${showLogDay} の割当根拠`} oC={() => setShowLogDay(null)} wide ch={
      ...
      {showRuleModal && (
        <Modal t="🏥 勤務割付システムのルールブック" oC={() => setShowRuleModal(false)} wide ch={
...
剰人員は定員オーバーでも汎用部屋（3号室等）に押し込まれます。</li>
            </ol>
            <div style={{ background: "#f8fafc", border: "1px solid #cbd5e1", padding: 16, borderRadius: 8, marginTop: 32 }}>
              <strong style={{ color: "#334155" }}>💡 最後に</strong><br/>もし「この部屋の負担が偏っている」場合は、人間が【設定画面】の「月担当」や「優先順位」を少し調整することで、意図に近いシフトを作れるようになります。
            </div>
