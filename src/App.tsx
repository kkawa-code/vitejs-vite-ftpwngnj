...
export class AutoAssigner {
  ...
  execute(): DayData {
    ...
    this.buildBlockMap();
    if (this.isSmartFix) { WORK_SECTIONS.forEach(sec => { let cur = split(this.dayCells[sec]); let nx = cur.filter(m => { const core = extractStaffName(m); const b = this.blockMap.get(core); if (ROLE_PLACEHOLDERS.includes(core)) return true; if (b === 'ALL') return false; if (b === 'AM' && (!m.includes("(") || m.includes("(AM)"))) return false; if (b === 'PM' && (!m.includes("(") || m.includes("(PM)"))) return false; const ttAM = this.timeTagMap.get(core); if (b === 'AM' && ttAM && m.includes(ttAM)) return false; return true; }); if (cur.length !== nx.length) { this.dayCells[sec] = join(nx); } }); }
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number, forcedNeedTag: string = "") {
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const c = pC[0], b = this.blockMap.get(c); let t = "", f = 1; if (b === 'AM') { t = this.timeTagMap.get(c) || "(PM)"; f = 0.5; this.blockMap.set(c, 'ALL'); } else if (b === 'PM') { t = this.timeTagMap.get(c) || "(AM)"; f = 0.5; this.blockMap.set(c, 'ALL'); } else { if (needTag) { t = needTag; f = 0.5; this.blockMap.set(c, needTag === "(AM)" ? 'AM' : 'PM'); } else if (eff.pmClosed) { t = "(AM)"; f = 0.5; this.blockMap.set(c, 'AM'); } else if (eff.amClosed) { t = "(PM)"; f = 0.5; this.blockMap.set(c, 'PM'); } else { t = ""; f = 1; this.blockMap.set(c, 'ALL'); } }
