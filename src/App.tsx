...
export const KEY_ALL_DAYS = "shifto_alldays_v297"; 
export const KEY_MONTHLY = "shifto_monthly_v297"; 
export const KEY_RULES = "shifto_rules_v297";
...
export function formatDayForDisplay(d: Date) { 
...
export class AutoAssigner {
  ...
  noHD(r: string, tag: string) { return this.isFDO(r) && (tag.includes("AM") || tag.includes("PM")); }
  iHB(st: string, sc: string) { if(!this.isFDO(sc)) return {h:false, m:false}; const fm=split(this.dayCells["不在"]); const a=fm.some(m=>extractStaffName(m)===st&&m.includes("(AM)")); const p=fm.some(m=>extractStaffName(m)===st&&m.includes("(PM)")); if(!a&&!p)return {h:false, m:false}; return {h:!isMonthlyMainStaff(sc,st,this.ctx.monthlyAssign), m:true}; }
  iHNC(s: string, r: string) { return this.prevDay ? split(this.ctx.customRules.noConsecutiveRooms).includes(r) && split(this.prevDay.cells[r]).map(extractStaffName).includes(s) : false; }
  gRSc(src: string, tgt: string, st?: string) { let sc=0; const l=split(this.ctx.customRules.supportTargetRoomsLowImpact); const h=split(this.ctx.customRules.supportTargetRoomsHighImpact); if(src===tgt)sc+=9999; if(l.includes(src))sc-=1000; if(h.includes(src))sc+=1000; sc+=this.getRoomDependencyCount(src)*100; const sm=split(this.dayCells[src]||""); const sL=split(this.ctx.customRules.supportStaffList).map(extractStaffName); if(sm.length>0&&sm.every(m=>sL.includes(extractStaffName(m))))sc+=5000; else{const a=sm.reduce((s,m)=>s+getStaffAmount(m),0); if(a<=1)sc+=500; else if(a<=2)sc+=200;} if(st&&h.includes(src)&&(this.blockMap.get(st)==='AM'||this.blockMap.get(st)==='PM'))sc+=300; if(this.clearSections.includes(src)||this.skipSections.includes(src))sc+=5000; return sc; }
  ...
  processPostTasks() {
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((r: any) => {
      ...
      while ((cA < tC || cP < tC) && uG1.length > 0) { const ci = uG1.findIndex(s => !this.isForbidden(s, r.target) && !this.iHB(s, r.target).h && !this.hasNGPair(s, cM.map(extractStaffName), false) && !this.iHNC(s, r.target) && !(r.target === "MMG" && !this.isMmgCapable(s)) && this.canAddKenmu(s, r.target) && !(cP >= tC && this.blockMap.get(s) === 'AM') && !(cA >= tC && this.blockMap.get(s) === 'PM') && !this.noHD(r.target, this.blockMap.get(s) || "")); if (ci === -1) break; const st = uG1[ci]; uG1.splice(ci, 1); const b = this.blockMap.get(st); let t = b === 'AM' ? "(PM)" : b === 'PM' ? "(AM)" : ""; if (!t) { if (cA >= tC) t = "(PM)"; else if (cP >= tC) t = "(AM)"; } this.blockMap.set(st, t === "" ? 'ALL' : t === "(AM)" ? 'PM' : 'AM'); cM.push(`${st}${t}`); if (t === "(AM)") cA++; else if (t === "(PM)") cP++; else { cA++; cP++; } this.addU(st, t ? 0.5 : 1); this.log(`🙌 [負担軽減] 余剰の ${st} を ${r.target} に専任配置`); } this.dayCells[r.target] = join(cM);
    });
    ...
    (this.ctx.customRules.smartKenmu || []).forEach((r: any) => {
      ...
      for (const tm of tM) { const tc = extractStaffName(tm); if (ROLE_PLACEHOLDERS.includes(tc) || ROOM_SECTIONS.some(rm => rm !== r.targetRoom && split(this.dayCells[rm]).map(extractStaffName).includes(tc))) continue; 
        ...
        for (const sSR of split(r.sourceRooms)) { const { r: sR, min } = parseRoomCond(sSR); const sMs = split(this.dayCells[sR]); if (sR === "透析後胸部" || (min > 0 && sMs.reduce((s, m) => s + getStaffAmount(m), 0) < min) || this.isForbidden(tc, sR) || this.iHB(tc, sR).h || !this.canAddKenmu(tc, sR, true)) continue;
          let cnds = sMs.filter(m => { const c = extractStaffName(m); return c !== tc && !ROLE_PLACEHOLDERS.includes(c) && !this.isForbidden(c, r.targetRoom) && !this.hasNGPair(c, tM.map(extractStaffName), false) && !this.iHNC(c, r.targetRoom) && (r.targetRoom === "MMG" ? this.isMmgCapable(c) : true) && this.canAddKenmu(c, r.targetRoom) && !this.noHD(r.targetRoom, m); });
  ...
  return (
    <div style={{ maxWidth: "98%", margin: "0 auto", padding: "24px", boxSizing: "border-box" }}>
      ...
      <div className="no-print" style={{ ...panelStyle(), display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, padding: "20px 32px", background: "linear-gradient(to right, #ffffff, #f8fafc)" }}>
        <h2 style={{ margin: 0, color: "#0f172a", fontSize: 26, fontWeight: 900 }}>勤務割付システム Ver 2.97</h2>
          ...
          <div style={{ borderLeft: "8px solid #94a3b8", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#f8fafc" border="#cbd5e1" color="#334155" icon="🙅" title="担当不可ルール">
              {(customRules.forbidden || []).map((rule: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: 16, borderBottom: "1px solid #e2e8f0", paddingBottom: 16 }}>
                    <Row><StaffSel v={rule.staff} oC={(v:any)=>updateRule("forbidden", idx, "staff", v)} list={activeGeneralStaff} /><DelBtn onClick={()=>removeRule("forbidden", idx)} /></Row>
            ...
            <RuleCard bg="#f0fdf4" border="#bbf7d0" color="#15803d" icon="🔒" title="専従ルール">
              {(customRules.fixed || []).map((rule: any, idx: number) => (
                  <Row key={idx}><StaffSel v={rule.staff} oC={(v:any)=>updateRule("fixed", idx, "staff", v)} list={activeGeneralStaff} /><RoomSel v={rule.section} oC={(v:any)=>updateRule("fixed", idx, "section", v)} list={ROOM_SECTIONS} /><DelBtn onClick={()=>removeRule("fixed", idx)} /></Row>
              ))}
              ...
              {(customRules.emergencies || []).map((em: any, idx: number) => (
                  <div key={idx} style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginBottom: '12px', background: '#fff', padding: '16px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                    ...
                    <div style={{ flex: '1 1 400px', display: 'flex', gap: '8px', alignItems: 'center', paddingLeft: '8px' }}>
                       ...
                       {em.type === 'change_capacity' && <><RoomSel v={em.section} oC={(v:any)=>updateRule("emergencies", idx, "section", v)} list={ROOM_SECTIONS} /> の定員を <NumInp v={em.newCapacity||1} onChange={(v:any)=>updateRule("emergencies", idx, "newCapacity", v)} w={60} /> 名にする</>}
        ...
        <Modal title="🏥 勤務割付システムのルールブック" onClose={() => setShowRuleModal(false)} wide>
          <div style={{ lineHeight: 1.8, fontSize: "16px", color: "#334155" }}>
            ...
            <ul style={{ paddingLeft: 24, marginBottom: 24 }}>
              ...
              <li style={{ marginBottom: 8 }}><strong>半休の終日専任室ブロック:</strong> 午前後休の人をCT/MRI等に配置することは原則ありません。（※ただしDSAなどの終日専任室には、AM/PMの半日だけ空いている人は入れません）</li>
            </ul>
