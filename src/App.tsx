...
interface CustomRules {
  staffList: string; receptionStaffList: string; supportStaffList: string; femaleStaffList: string; supportTargetRooms: string; 
...
const DEFAULT_RULES: CustomRules = { 
  staffList: "", receptionStaffList: "", supportStaffList: "", femaleStaffList: "", supportTargetRooms: "2号室, 3号室", 
...
const SectionEditor = ({ section, value, activeStaff, onChange, noTime = false, customOptions = [], onAddHelp, dayCells, helpRoomMap, helpTimeMap }: any) => {
          ...
          return (
            <div key={i} style={{ background: isPlaceholder ? "#fef08a" : (noTime && !isFuzai ? "#f1f5f9" : "#e0f2fe"), color: isPlaceholder ? "#a16207" : (noTime && !isFuzai ? "#334155" : "#0369a1"), borderRadius: 16, padding: "8px 12px 8px 14px", fontSize: 15, display: "flex", alignItems: "center", gap: 6, border: `1px solid ${isPlaceholder ? "#fde047" : (noTime && !isFuzai ? "#cbd5e1" : "#bae6fd")}`, fontWeight: 700 }}>
              ...
              {isFuzai && currentMod !== "" && onAddHelp && (() => {
                ...
                const foundRm = mappedRoom
                  || (mappedTime && dayCells ? HELP_RMS.find((r:string) => split(dayCells[r]||"").some((m:string) => m === `${r}枠${mappedTime}`)) : null)
                  || (dayCells ? HELP_RMS.find((r:string) => split(dayCells[r]||"").some((m:string) => extractStaffName(m) === coreName)) : null)
                  || null;
                return (
...
class AutoAssigner {
  ...
  canAddKenmu(staff: string, targetRoom: string, bypassExclusiveForSource: boolean = false, ignoreRoomLimit: boolean = false): boolean {
    const limit = this.ctx.customRules.alertMaxKenmu || 3;
    const currentRoomCount = this.getTodayRoomCount(staff);
    if (!ignoreRoomLimit && !split(this.dayCells[targetRoom] || "").map(extractStaffName).includes(staff) && currentRoomCount >= limit) return false;
  ...
  canAddConfiguredKenmu(staff: string, targetRoom: string, bypassExclusiveForSource: boolean = false): boolean {
    const base = this.canAddKenmu(staff, targetRoom, bypassExclusiveForSource, true);
    return base;
  }

  getRoomsForStaff(staff: string): string[] {
    return ROOM_SECTIONS.filter(r => split(this.dayCells[r] || "").map(extractStaffName).includes(staff));
  }

  getExclusiveCounterpart(room: string): string | null {
    const pair = (this.ctx.customRules.kenmuPairs || []).find((p: any) => p.isExclusive && (p.s1 === room || p.s2 === room));
    if (!pair) return null;
    return pair.s1 === room ? pair.s2 : pair.s2 === room ? pair.s1 : null;
  }

  respectsExclusivePairsForProjectedRooms(rooms: string[]): boolean {
    const exclusivePairs = (this.ctx.customRules.kenmuPairs || []).filter((p: any) => p.isExclusive);
    for (const p of exclusivePairs) {
      const hasPair = rooms.includes(p.s1) || rooms.includes(p.s2);
      if (!hasPair) continue;
      const hasOutside = rooms.some(r => r !== p.s1 && r !== p.s2);
  ...
  getProjectedRoomsForSwapSource(staff: string, srcRoom: string, triggerRoom: string, targetRoom: string): string[] {
    let rooms = this.getRoomsForStaff(staff).filter(r => r !== srcRoom);
    const counterpart = this.getExclusiveCounterpart(srcRoom);
    if (counterpart && targetRoom !== srcRoom && targetRoom !== counterpart && triggerRoom !== srcRoom && triggerRoom !== counterpart) {
      rooms = rooms.filter(r => r !== counterpart);
    }
    if (!rooms.includes(triggerRoom)) rooms.push(triggerRoom);
    if (!rooms.includes(targetRoom)) rooms.push(targetRoom);
    return Array.from(new Set(rooms));
  }

  getProjectedRoomsForSwapKick(staff: string, triggerRoom: string, srcRoom: string): string[] {
    const rooms = this.getRoomsForStaff(staff).filter(r => r !== triggerRoom);
    if (!rooms.includes(srcRoom)) rooms.push(srcRoom);
    return Array.from(new Set(rooms));
  }
  ...
  isHalfDayRestrictedStaff(staff: string): boolean {
    const b = this.blockMap.get(staff);
    return b === "AM" || b === "PM" || this.timeTagMap.has(staff);
  }

  canAssignCoreModality(staff: string, section: string): boolean {
    if (!["CT", "MRI"].includes(section)) return true;
    return !this.isHalfDayRestrictedStaff(staff);
  }

  getEffectiveFemaleStaffList(): string[] {
    const explicit = split(this.ctx.customRules.femaleStaffList || "").map(extractStaffName);
    if (explicit.length > 0) return explicit;
    return split(this.ctx.monthlyAssign.MMG || "").map(extractStaffName);
  }

  getPreferredTagForStaff(staff: string): string {
    const b = this.blockMap.get(staff); const tt = this.timeTagMap.get(staff);
    if (b === 'AM') return '(PM)';
    if (b === 'PM') return tt || '(AM)';
    return '';
  }
  getTagAmount(tag: string): number { return tag ? 0.5 : 1; }
  ...
  execute(): DayData {
    ...
    if (this.isSmartFix) {
      ...
      WORK_SECTIONS.forEach(sec => {
        ...
        let next = current.filter(m => { const core = extractStaffName(m); const b = this.blockMap.get(core); if (ROLE_PLACEHOLDERS.includes(core)) return true; if (b === 'ALL') return false; const tt = this.timeTagMap.get(core); if (tt && m.includes(tt)) return true; if (b === 'AM' && (!m.includes("(") || m.includes("(AM)"))) return false; if (b === 'PM' && (!m.includes("(") || m.includes("(PM)"))) return false; if (b === 'PM' && tt && m.includes("(AM)")) return false; return true; });
    ...
    if (this.isSmartFix) {
      ...
      priority.forEach((room: string) => {
         ...
         while (getAmt(current) < eff.cap) {
            const freeStaff = sortedAvail.find((s: string) => { 
                ...
                if (!this.canAssignCoreModality(s, room)) return false; if (this.isForbidden(s, room)) return false; if (room === "MMG" && !this.isMmgCapable(s)) return false; if (!this.canAddKenmu(s, room)) return false; if (this.isHardNoConsecutive(s, room)) return false; if (isFixedToAny(s)) return false; 
      ...
      (this.ctx.customRules.fixed || []).forEach((rule: any) => { 
          ...
          if (!core || !rule.section || !this.initialAvailAll.includes(core) || this.isUsed(core) || this.isForbidden(core, rule.section) || !this.canAssignCoreModality(core, rule.section)) return; 
          ...
          const tag = this.getPreferredTagForStaff(core); 
          this.blockMap.set(core, 'ALL'); this.dayCells[rule.section] = join([...current, `${core}${tag}`]); this.addU(core, this.getTagAmount(tag)); this.log(`🔒 [専従] ${core} を ${rule.section} に固定配置しました`); 
      });
      this.staffAssignments.forEach((rule: any) => { 
          ...
          if (current.map(extractStaffName).includes(core) || this.hasNGPair(core, current.map(extractStaffName), false)) return; 
          const tag = this.getPreferredTagForStaff(core); 
          this.blockMap.set(core, 'ALL'); this.dayCells[rule.section] = join([...current, `${core}${tag}`]); this.addU(core, this.getTagAmount(tag)); this.log(`🚨 [緊急専従] 人数不足のため ${core} を ${rule.section} に強制配置しました`); 
      });
      Object.values(this.roleAssignments).forEach((ra: any) => { if (this.skipSections.includes(ra.section)) return; const candidates = split(this.ctx.monthlyAssign[ra.role] || "").map(extractStaffName); const targetAvail = ["受付"].includes(ra.role) ? this.initialAvailReception : this.initialAvailGeneral; const staff = candidates.find(s => targetAvail.includes(s) && !this.isUsed(s) && !this.isForbidden(s, ra.section) && this.canAssignCoreModality(s, ra.section)); if (staff && !split(this.dayCells[ra.section]).map(extractStaffName).includes(staff)) { const tag = this.getPreferredTagForStaff(staff); this.blockMap.set(staff, 'ALL'); this.dayCells[ra.section] = join([...split(this.dayCells[ra.section]), `${staff}${tag}`]); this.addU(staff, this.getTagAmount(tag)); this.log(`📌 [緊急役割] ${staff} を ${ra.section} に配置しました`); } });
      (this.ctx.customRules.substitutes || []).forEach((sub: any) => {
         ...
         if (trigger) {
           const fallbackStaff = split(sub.subs).map(extractStaffName).filter(s => this.initialAvailGeneral.includes(s) && !this.isUsed(s) && !this.isForbidden(s, sub.section) && this.canAssignCoreModality(s, sub.section));
           if (fallbackStaff.length > 0) {
             ...
             for (const f of fallbackStaff) {
               ...
               if (!this.hasNGPair(f, currentRoomMembers.map(extractStaffName), false) && currentRoomMembers.length < 6) {
                 ...
                 const tag = this.getPreferredTagForStaff(f); this.blockMap.set(f, 'ALL');
                 this.dayCells[sub.section] = join([...currentRoomMembers, `${f}${tag}`]); this.addU(f, this.getTagAmount(tag)); this.log(`🔄 [代打] ${sub.target} が不在のため、${f} を ${sub.section} に配置しました`); break;
               }
             }
           }
         }
           ...
           if (currentTSec.includes(s1) && currentTSec.includes(s2)) {
             ...
             for (const room of allowedRooms) {
               ...
               if (getAmt(currentRoom) < actualCap) {
                 ...
                 const tag = this.getPreferredTagForStaff(s2); 
      ...
      PRIORITY_LIST.forEach((room: string) => {
        ...
        let untimedPlaceholderCount = 0;
        if (placeholders.length > 0) {
          placeholders.forEach(p => {
            const core = extractStaffName(p);
            const tag = p.substring(core.length);
            if (tag) placeholderTags.push(tag);
            else untimedPlaceholderCount += 1;
          });
          targetCount += untimedPlaceholderCount;
        ...
        if (room === "受付") {
          ...
          for (const name of uketsukeMonthly) { if (this.initialAvailAll.includes(name) && !this.isUsed(name) && !currentUketsuke.map(extractStaffName).includes(name)) { const b = this.blockMap.get(name); if (b === 'ALL') continue; const tag = this.getPreferredTagForStaff(name); currentUketsuke.push(`${name}${tag}`); this.addU(name, this.getTagAmount(tag)); this.blockMap.set(name, 'ALL'); } }
          ...
          if (neededUketsuke > 0 && !linkedTargetRooms.includes(room)) {
             ...
             pickedUketsuke.forEach((name: string) => { const tag = this.getPreferredTagForStaff(name); currentUketsuke.push(`${name}${tag}`); this.addU(name, this.getTagAmount(tag)); this.blockMap.set(name, 'ALL'); });
          }
          ...
          if (!((this.ctx.customRules.linkedRooms || []).some((r: any) => r.target === room)) && !hasPartnerFilled) {
            this.fill(candidates, room, preferredList, targetCount);
            placeholderTags.forEach((tag: string) => {
              const currentAmount = split(this.dayCells[room]).reduce((sum, m) => sum + getStaffAmount(m), 0);
              this.fill(candidates, room, preferredList, currentAmount + 0.5, tag);
            });
          } else { this.log(`⏭️ [専任スキップ] ${room} は兼務ルールの対象のため、メイン専任配置をスキップ`); }
        }
      });
    }
  ...
  buildBlockMap() {
    ...
    split(this.dayCells["不在"]).forEach(m => {
      ...
      if (m.includes("(AM)")) this.blockMap.set(core, 'AM');
        ...
        this.blockMap.set(core, 'AM');
      } else if (m.match(/\(\d.*〜\)/)) {
        this.blockMap.set(core, 'PM');
        const t = m.match(/\((\d+:\d+)〜\)/)?.[1];
        if (t) this.timeTagMap.set(core, `(〜${t})`);
  ...
  evaluateEmergencies() {
    const staffWeight = (staff: string) => {
      const b = this.blockMap.get(staff);
      if (b === 'ALL') return 0;
      if (b === 'AM' || b === 'PM') return 0.5;
      if (this.timeTagMap.has(staff)) return 0.5;
      return 1;
    };
    const tempAvailCount = this.ctx.activeGeneralStaff.reduce((sum, s) => sum + staffWeight(s), 0);
  ...
  cleanUpDayCells() {
    Object.keys(this.dayCells).forEach(sec => {
      ...
      let members = split(this.dayCells[sec]).map(m => {
        const core = extractStaffName(m); if (ROLE_PLACEHOLDERS.includes(core)) return m; const block = this.blockMap.get(core); const tt = this.timeTagMap.get(core);
        if (block === 'ALL') return null;
        if (tt && m.includes(tt)) return m;
        if (block === 'AM' && m.includes('(AM)')) return null;
        if (block === 'PM' && m.includes('(PM)')) return null;
        if (block === 'PM' && tt && m.includes('(AM)')) return `${core}${tt}`;
        if (block === 'AM' && !m.includes('(PM)') && !m.match(/\(.*\)/)) return `${core}(PM)`;
        if (block === 'PM' && !m.includes('(AM)') && !m.match(/\(.*\)/)) return `${core}${tt || '(AM)'}`;
        return m;
  ...
  fill(availList: string[], section: string, preferredList: string[], targetCount: number, forcedNeedTag: string = "") {
    ...
    const isUntilTag = (tag: string) => /^\(〜\d{1,2}:\d{2}\)$/.test(tag);
    const isFromTag = (tag: string) => /^\(\d{1,2}:\d{2}〜\)$/.test(tag);
    const tagMinutes = (tag: string) => {
      const m = tag.match(/(\d{1,2}):(\d{2})/);
      return m ? Number(m[1]) * 60 + Number(m[2]) : null;
    };
    const nextBlockForTag = (current: string | undefined, tag: string) => {
      const amLike = tag === "(AM)" || isUntilTag(tag);
      if (amLike) return (current === 'PM' || current === 'ALL') ? 'ALL' : 'AM';
      return (current === 'AM' || current === 'ALL') ? 'ALL' : 'PM';
    };
    ...
    while (getCurrentAmount(current) < eff.cap) {
      ...
      const getFilterReason = (name: string): RejectReason | null => {
         ...
         if (!this.canAssignCoreModality(name, section)) return { hard: true, msg: "半日者はCT/MRI不可" };
         ...
         const tt = this.timeTagMap.get(name);
         
         if (needTag && b === 'NONE' && ["(AM)", "(PM)"].includes(needTag)) {
           if (!eff.pmClosed && !eff.amClosed) {
         ...
         if (isFromTag(needTag) && b === 'PM') return { hard: true, msg: "午後帯不可" };
         if (isUntilTag(needTag) && b === 'AM') return { hard: true, msg: "午前帯不可" };
         if (isUntilTag(needTag) && b === 'PM' && tt) {
           const needEnd = tagMinutes(needTag); const haveEnd = tagMinutes(tt);
           if (needEnd !== null && haveEnd !== null && haveEnd < needEnd) return { hard: true, msg: "時刻不足" };
         }
      ...
      const sortCandidates = (candidates: string[]) => {
         ...
         return [...candidates].sort((a, b) => {
             ...
             if (needTag === "") { if (bA === 'NONE') scoreA += 200; else if (hasAmFree && hasPmFree && (bA === 'AM' || bA === 'PM')) scoreA += 100; }
             else { if ((needTag === "(AM)" || isUntilTag(needTag)) && bA === 'PM') scoreA += 200; if ((needTag === "(PM)" || isFromTag(needTag)) && bA === 'AM') scoreA += 200; if (bA === 'NONE') scoreA += 100; }
             if (needTag === "") { if (bB === 'NONE') scoreB += 200; else if (hasAmFree && hasPmFree && (bB === 'AM' || bB === 'PM')) scoreB += 100; }
             else { if ((needTag === "(AM)" || isUntilTag(needTag)) && bB === 'PM') scoreB += 200; if ((needTag === "(PM)" || isFromTag(needTag)) && bB === 'AM') scoreB += 200; if (bB === 'NONE') scoreB += 100; }
      ...
      if (forcedNeedTag) {
        tag = needTag; f = 0.5; this.blockMap.set(core, nextBlockForTag(block, tag));
      } else if (block === 'AM') { tag = "(PM)"; f = 0.5; this.blockMap.set(core, 'ALL'); } else if (block === 'PM') { tag = "(AM)"; f = 0.5; this.blockMap.set(core, 'ALL'); } else { if (needTag) { tag = needTag; f = 0.5; this.blockMap.set(core, needTag === "(AM)" ? 'AM' : 'PM'); } else { tag = ""; f = 1; this.blockMap.set(core, 'ALL'); } }
  ...
  processPostTasks() {
    ...
    this.initialAvailSupport.forEach(staff => {
      ...
      for (const room of targets) {
        ...
        if (current.length > 0 && !current.map(extractStaffName).includes(staff) && !this.hasNGPair(staff, current.map(extractStaffName), false) && !this.isHardNoConsecutive(staff, room)) {
          const tag = this.getPreferredTagForStaff(staff);
          this.dayCells[room] = join([...current, `${staff}${tag}`]); this.addU(staff, this.getTagAmount(tag)); this.blockMap.set(staff, 'ALL');
      ...
      if (!assigned) {
        for (const room of targets) {
          ...
          if (current.length === 0 && !this.isHardNoConsecutive(staff, room) && this.canAddKenmu(staff, room)) {
             const tag = this.getPreferredTagForStaff(staff);
             this.dayCells[room] = `${staff}${tag}`; this.addU(staff, this.getTagAmount(tag)); this.blockMap.set(staff, 'ALL');
    ...
    (this.ctx.customRules.swapRules || []).forEach((rule: any) => {
      ...
      const triggerCanTarget = triggerMembers.some(m => {
        ...
        if (ROLE_PLACEHOLDERS.includes(c) || this.isForbidden(c, targetRoom) || this.isHardNoConsecutive(c, targetRoom) || this.isHalfDayBlockedForFullDayRoom(c, targetRoom).hard) return false;
        return this.canAddConfiguredKenmu(c, targetRoom, true);
      });
      if (triggerCanTarget) return;

      const swapSources = split(sourceRooms);
      this.log(`🧭 [交換候補] ${targetRoom} の交換元を ${swapSources.map(s=>parseRoomCond(s).r).join(" → ")} の順で評価しました`);
      let swapped = false;
      for (const srcStrRoom of swapSources) {
        const { r: srcRoom } = parseRoomCond(srcStrRoom);
        ...
        let srcCands = srcMembers.filter(m => {
          ...
          if (ROLE_PLACEHOLDERS.includes(core)) return false;
          if (m.includes("17:")) return false;
          if (!this.canAssignCoreModality(core, triggerRoom) || !this.canAssignCoreModality(core, targetRoom)) return false;
          if (this.isForbidden(core, targetRoom) || this.isForbidden(core, triggerRoom)) return false;
        ...
        for (const srcM of srcCands) {
          ...
          const nextTriggerMembers = triggerMembers.map(m => m === targetToKick ? srcM : m);
          const kickReplacement = kickCore + (srcM.includes("(AM)") ? "(AM)" : srcM.includes("(PM)") ? "(PM)" : "");
          const nextSourceMembers = srcMembers.map(m => m === srcM ? kickReplacement : m);
          let targetMembers = split(this.dayCells[targetRoom]);
          const counterpart = this.getExclusiveCounterpart(srcRoom);
          if (counterpart && targetRoom !== srcRoom && targetRoom !== counterpart && triggerRoom !== srcRoom && triggerRoom !== counterpart) {
            const counterpartMembers = split(this.dayCells[counterpart] || "");
            const removed = counterpartMembers.filter(m => extractStaffName(m) === srcCore);
            if (removed.length > 0) {
              this.dayCells[counterpart] = join(counterpartMembers.filter(m => extractStaffName(m) !== srcCore));
              removed.forEach(m => {
                this.assignCounts[srcCore] = Math.max(0, (this.assignCounts[srcCore] || 0) - getStaffAmount(m));
              });
              this.log(`🔧 [専従調整] ${srcCore} を ${counterpart} から外して ${targetRoom} へ回しました`);
            }
          }
          if (!targetMembers.map(extractStaffName).includes(srcCore)) {
            targetMembers = [...targetMembers, srcM];
            this.addU(srcCore, getStaffAmount(srcM));
            this.updateBlockMapAfterKenmu(srcCore, srcM);
          }
          this.dayCells[triggerRoom] = join(nextTriggerMembers);
          this.dayCells[srcRoom] = join(nextSourceMembers);
          this.dayCells[targetRoom] = join(targetMembers);
          this.log(`🔄 [交換] ${srcRoom} の ${srcCore} を ${triggerRoom} に入れ替え、${targetRoom} にも配置しました（${kickCore} → ${srcRoom}）`);
          swapped = true;
          break;
        }
        if (swapped) break;
      }
      if (!swapped) this.log(`⚠️ [交換失敗] ${targetRoom} のための交換候補が見つかりませんでした`);
    });
    ...
    (this.ctx.customRules.linkedRooms || []).forEach((rule: any) => {
      ...
      while ((curAm < targetCap || curPm < targetCap) && uGen1.length > 0) {
        ...
        if (tag === "") {
           if (curAm >= targetCap) tag = "(PM)";
           else if (curPm >= targetCap) tag = "(AM)";
        }
        
        this.blockMap.set(staff, tag === "" ? 'ALL' : (tag === "(AM)" ? 'PM' : 'AM'));
        currentMems.push(`${staff}${tag}`); 
        if (tag==="(AM)") curAm++; else if (tag==="(PM)") curPm++; else { curAm++; curPm++; } 
    ...
    const processKenmu = (sourceMems: string[], targetMems: string[], targetRoom: string) => {
       ...
       for (const m of sourceMems) {
          ...
          const core = extractStaffName(m);
          if (targetCores.includes(core) || m.includes("17:") || m.includes("19:") || this.isForbidden(core, targetRoom) || this.isHardNoConsecutive(core, targetRoom) || this.isHalfDayBlockedForFullDayRoom(core, targetRoom).hard || this.hasNGPair(core, targetCores, false) || !this.canAddConfiguredKenmu(core, targetRoom)) continue;
    ...
    ROOM_SECTIONS.forEach(targetRoom => {
      ...
      let currentMems = split(this.dayCells[targetRoom]); let curAm = 0; let curPm = 0; currentMems.forEach(x => { if (x.includes("(AM)")) curAm += 1; else if (x.includes("(PM)")) curPm += 1; else { curAm += 1; curPm += 1; } });
      const isOnlySupport = currentMems.length > 0 && currentMems.every(m => supportStaffList.includes(extractStaffName(m)));
      if (isOnlySupport) { curAm = 0; curPm = 0; this.log(`💡 [救済発動] ${targetRoom} はサポート要員しかいないため、応援を兼務で呼びます`); }
    ...
    uGen2.forEach(staff => {
      const b = this.blockMap.get(staff); if (b === 'ALL') return; let tag = this.getPreferredTagForStaff(staff); let assigned = false;
    ...
    if (!this.skipSections.includes("昼当番")) {
      ...
      const femaleStaffPool = this.getEffectiveFemaleStaffList();
      ...
      if (femaleStaffPool.length > 0 && !currentLunch.some(n => femaleStaffPool.includes(n))) {
        const femaleSearchRooms = Array.from(new Set(["MMG", "RI", ...split(this.ctx.customRules.lunchPrioritySections ?? "RI,1号室,2号室,3号室,5号室,CT"), ...ROOM_SECTIONS]));
        let femalePick: string | null = null;
        for (const room of femaleSearchRooms) {
          const picked = split(this.dayCells[room] || "").map(extractStaffName).find(n => femaleStaffPool.includes(n) && !currentLunch.includes(n) && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, currentLunch, false));
          if (picked) { femalePick = picked; break; }
        }
        if (!femalePick) femalePick = this.initialAvailGeneral.find(n => femaleStaffPool.includes(n) && !currentLunch.includes(n) && !this.isForbidden(n, "昼当番") && !this.hasNGPair(n, currentLunch, false)) || null;
        if (femalePick) {
          if (currentLunch.length < lunchTarget) currentLunch.push(femalePick);
          else {
            const replaceIdx = currentLunch.findIndex(n => !femaleStaffPool.includes(n));
            if (replaceIdx >= 0) currentLunch[replaceIdx] = femalePick;
          }
          this.log(`👩 [昼当番(女性確保)] 女性枠として ${femalePick} を昼当番に配置しました`);
        } else {
          this.log(`⚠️ [昼当番(女性確保失敗)] 女性枠を満たせる候補が見つかりませんでした`);
        }
      }
...
export default function App(): any {
  ...
  const parseStoredMap = (raw: string): Record<string, string> => {
    if (!raw) return {};
    try { return JSON.parse(raw.replace(/、/g, ",")); } catch { return {}; }
  };
  const stringifyStoredMap = (obj: Record<string, string>) => JSON.stringify(obj);
  const sanitizeAbsenceHelpMeta = (cells: any) => {
    const absentNames = new Set(split(cells["不在"] || "").map((m: string) => extractStaffName(m)));
    const helpMap = parseStoredMap(cells["__absenceHelp"] || "");
    const helpRoomMap = parseStoredMap(cells["__absenceHelpRooms"] || "");
    const nextHelp: Record<string, string> = {};
    const nextRooms: Record<string, string> = {};
    Object.keys(helpMap).forEach(name => { if (absentNames.has(name)) nextHelp[name] = helpMap[name]; });
    Object.keys(helpRoomMap).forEach(name => { if (absentNames.has(name)) nextRooms[name] = helpRoomMap[name]; });
    cells["__absenceHelp"] = stringifyStoredMap(nextHelp);
    cells["__absenceHelpRooms"] = stringifyStoredMap(nextRooms);
    return cells;
  };
  const updateDay = (k: string, v: string) => { setAllDaysWithHistory((prev: any) => { const nextCells = { ...(prev[sel] || {}), [k]: v }; if (k === "不在") sanitizeAbsenceHelpMeta(nextCells); const nextState = { ...prev, [sel]: nextCells }; if (k === "入り") { const idx = days.findIndex(d => d.id === sel); if (idx >= 0 && idx < days.length - 1) { const nextDayId = days[idx + 1].id; const currentAke = split((prev[nextDayId] || {})["明け"]).filter(m => !split(v).includes(m)); nextState[nextDayId] = { ...(prev[nextDayId] || {}), "明け": join([...currentAke, ...split(v)]) }; } } return nextState; }); };
  ...
  const preserveTimedRoomPlaceholders = (raw: string) => join(split(raw || "").filter((m: string) => ROLE_PLACEHOLDERS.includes(extractStaffName(m))));
  const handleClearGroupDay = (title: string, sections: string[]) => { if (window.confirm(`${cur.label} の「${title}」をクリアしますか？`)) { setAllDaysWithHistory((prev: any) => { const nextCells = { ...(prev[cur.id] || cur.cells) }; sections.forEach(sec => { nextCells[sec] = preserveTimedRoomPlaceholders(nextCells[sec]); }); return { ...prev, [cur.id]: nextCells }; }); } };
  const handleClearGroupWeek = (title: string, sections: string[]) => { if (window.confirm(`表示中の「${title}」を1週間分すべてクリアしますか？`)) { setAllDaysWithHistory((prev: any) => { const nextState = { ...prev }; days.forEach(d => { const nextCells = { ...(prev[d.id] || d.cells) }; sections.forEach(sec => { nextCells[sec] = preserveTimedRoomPlaceholders(nextCells[sec]); }); nextState[d.id] = nextCells; }); return nextState; }); } };
  const handleClearWorkDay = () => { if (window.confirm(`${cur.label} の「モダリティ」と「一般撮影・透視・その他」をクリアしますか？`)) { const workSections = [...RENDER_GROUPS[1].sections, ...RENDER_GROUPS[2].sections]; setAllDaysWithHistory((prev: any) => { const nextCells = { ...(prev[cur.id] || cur.cells) }; workSections.forEach(sec => { nextCells[sec] = preserveTimedRoomPlaceholders(nextCells[sec]); }); return { ...prev, [cur.id]: nextCells }; }); } };
  const handleClearWorkWeek = () => { if (window.confirm(`表示中の「モダリティ」と「一般撮影・透視・その他」を1週間分すべてクリアしますか？`)) { const workSections = [...RENDER_GROUPS[1].sections, ...RENDER_GROUPS[2].sections]; setAllDaysWithHistory((prev: any) => { const nextState = { ...prev }; days.forEach(d => { const nextCells = { ...(prev[d.id] || d.cells) }; workSections.forEach(sec => { nextCells[sec] = preserveTimedRoomPlaceholders(nextCells[sec]); }); nextState[d.id] = nextCells; }); return nextState; }); } };
         ...
         {/* ===================== フェーズ5 ===================== */}
          <div style={{ borderLeft: "8px solid #8b5cf6", paddingLeft: 24, marginBottom: 40 }}>
            ...
            <RuleCard bg="#eef2ff" border="#c7d2fe" color="#4338ca" icon="🍱" title="昼当番ルール">
              ...
              <div style={{ background: "#fff", padding: 16, borderRadius: 10, border: "1px solid #c7d2fe", marginBottom: 20 }}>
                <div style={{ fontWeight: 800, color: "#4338ca", marginBottom: 8 }}>女性スタッフ（未設定ならMMG担当を自動使用）</div>
                <MultiPicker selected={customRules.femaleStaffList ?? ""} onChange={(v: string) => setCustomRules({...customRules, femaleStaffList: v})} options={allStaff} />
                <div style={{ fontSize: 12, color: "#6366f1", marginTop: 4 }}>昼当番には最低1人、このリストのスタッフを入れます。空ならMMG担当者を女性グループとして扱います。</div>
              </div>
